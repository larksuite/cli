...
- ✅ 在某个群的范围内按关键词找机器人(`--chat-ids`;它换的是搜索范围而不是过滤,见下)
- ❌ 想列出某个群的全部机器人 → 走群成员列表([`lark-im`](../../lark-im/SKILL.md)),本命令必须带关键词
...
**必须给关键词**:`--query` 或 `--queries` 至少一个。`--chat-ids` 和 `--has-chatted` 只能配合关键词使用,不能独立枚举。
...
**群内的机器人在租户级搜索里可能完全不可见,本命令下只有指定群才搜得到。** 所以 `--chat-ids` 的结果不是无过滤结果的子集,它能让你找到 `--query` 单独怎么都搜不出来的机器人。实测过的两种情形:
