...
| 查日历/日程或未来时间的会议 | [lark-calendar](../lark-calendar/GUIDE.md) |
| 查"今天有哪些会议" | `vc +search`（已结束）+ lark-calendar（未开始），合并展示 |
| Agent 真实入会/离会、会中实时事件 | [lark-vc-agent](../lark-vc-agent/GUIDE.md) |
| 本地音视频文件转纪要/逐字稿 | 先走 [lark-minutes](../lark-minutes/GUIDE.md) 上传，再回 `vc +notes --minute-tokens` |
...
| **进行中会议**的实时事件流（转写、聊天、共享、会中加入/离开）| `vc +meeting-events` | [`lark-vc-agent`](../lark-vc-agent/GUIDE.md) |
| **Agent 真实入会 / 离会** | `vc +meeting-join` / `vc +meeting-leave` | [`lark-vc-agent`](../lark-vc-agent/GUIDE.md) |
...
- Agent 真实入会/离会、会中实时事件 → [lark-vc-agent](../lark-vc-agent/GUIDE.md)
- 本地音视频文件转纪要/逐字稿 → [lark-minutes](../lark-minutes/GUIDE.md)（上传后回 `vc +notes`）
- 妙记搜索/下载/上传/重命名/替换说话人 → [lark-minutes](../lark-minutes/GUIDE.md)
