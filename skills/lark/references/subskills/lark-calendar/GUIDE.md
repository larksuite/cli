...
开始前先读 [`../lark-shared/GUIDE.md`](../lark-shared/GUIDE.md)（认证、权限处理）。
...
| 用户意图 | 路由到 |
|----------|--------|
| 查询过去的会议（"昨天的会议""上周的会"） | [`../lark-vc/GUIDE.md`](../lark-vc/GUIDE.md)（会议数据含即时会议，仅查日程会遗漏） |
...
- 查询过去的视频会议记录 → [lark-vc](../lark-vc/GUIDE.md)
- 待办任务管理 → [lark-task](../lark-task/GUIDE.md)
- 会议室物理设施管理 → 管理员后台
