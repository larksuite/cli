...
> **Prerequisite:** Read [`../GUIDE.md`](../GUIDE.md) first for the `event consume` essentials (commands, subprocess contract, jq usage).
...
| `kill -9` | **Skips unsubscribe → server-side subscription leaks**, may cause `subscription already exists` or duplicate delivery on next consume. See GUIDE.md "Never `kill -9`". |
