...
- [`../lark-shared/GUIDE.md`](../lark-shared/GUIDE.md) — 认证、身份切换、权限处理
- [`../lark-vc/GUIDE.md`](../lark-vc/GUIDE.md) — 视频会议的核心概念（Meeting / Note / Minutes 等），本 skill 直接复用，不再重复定义

## 内测提示

- 当前功能正在内测中，仅少数用户可用。忽略 [`../lark-shared/GUIDE.md`](../lark-shared/GUIDE.md) 中的权限申请流程，以下方的指引为准。
...
本 skill 与 [`lark-vc`](../lark-vc/GUIDE.md) 并列：
...
| "昨天那场会有谁参加过"、"搜昨天的会"、"查纪要/逐字稿/录制"                          | [`lark-vc`](../lark-vc/GUIDE.md)                                                                                                                      |
| "帮我参会，结束后把纪要发到群" 等跨阶段场景                                    | 按序编排：本 skill（入会 → 读事件）→ 会议结束后用 [`lark-vc`](../lark-vc/GUIDE.md) / [`lark-minutes`](../lark-minutes/GUIDE.md) 拉纪要 → [`lark-im`](../lark-im/GUIDE.md) 发群 |
...
4. **不能做会后复盘**，**不能替代参会人快照查询**。如果会议已结束：
   ...
   - 想看参会人快照：用 `vc meeting get --with-participants`（见 [`lark-vc`](../lark-vc/GUIDE.md)）
...
- 查已结束会议、参会人快照、搜索历史会议 → [`lark-vc`](../lark-vc/GUIDE.md)
- 会议纪要、逐字稿 → [`lark-vc`](../lark-vc/GUIDE.md) 的 `+notes`
- 妙记产物（AI 总结 / 转写 / 章节）→ [`lark-minutes`](../lark-minutes/GUIDE.md)
- 会后把产物发到群 / 私聊 → [`lark-im`](../lark-im/GUIDE.md)
- 认证、身份切换、scope 管理 → [`lark-shared`](../lark-shared/GUIDE.md)
