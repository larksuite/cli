...
返回中可能含 `<sheet>`、`<bitable>`、`<cite file-type="sheets|bitable">`。内部数据无法通过 `docs +fetch --api-version v2` 获取，提取 `token` 等属性后切到 [`lark-sheets`](../../lark-sheets/GUIDE.md) / [`lark-base`](../../lark-base/GUIDE.md) 下钻，详见 [GUIDE.md 快速决策](../GUIDE.md) 路由表。
