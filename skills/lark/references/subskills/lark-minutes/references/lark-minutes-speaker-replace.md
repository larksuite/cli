...
> **重要**：`--from-user-id` 和 `--to-user-id` 仅支持 `ou_` 开头的用户 ID，**不支持直接传姓名**。如果用户只给了姓名，请先用 [lark-contact](../../lark-contact/GUIDE.md) 把姓名解析成 `open_id`，再调用本命令。
...
- [lark-minutes](../GUIDE.md) -- 妙记相关功能说明
