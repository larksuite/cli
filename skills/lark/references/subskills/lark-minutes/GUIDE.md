...
| "这个妙记的逐字稿/总结/待办/章节" | [lark-vc](../lark-vc/GUIDE.md)（`vc +notes --minute-tokens`） |
| "把音视频文件转成纪要/逐字稿/文字稿" | 先本 skill（`+upload`），再 [lark-vc](../lark-vc/GUIDE.md)（`vc +notes --minute-tokens`） |
| 用户同时提到"会议/开会"和"妙记" | 先 [lark-vc](../lark-vc/GUIDE.md)（`+search` → `+recording`），再本 skill |
...
4. 如果是会议的妙记，应优先通过 [lark-vc](../lark-vc/GUIDE.md) 定位会议并获取 `minute_token`。
...
> **跨 skill 路由**：逐字稿、AI 总结、待办、章节等纪要内容由 [lark-vc](../lark-vc/GUIDE.md) 的 `+notes` 命令提供
> **读 vs 写**：`vc +notes` 只负责**读取** AI 产物。用户要**新建 / 修改 / 删除**妙记内的 AI 待办或替换 AI 总结，见下文第 6 节，**不要**走 [lark-task](../lark-task/GUIDE.md)。
...
| 我的任务清单 | — | 走 [lark-task](../lark-task/GUIDE.md) |
...
> - 用户只是想看"我的妙记 / 某段时间内的妙记 / 妙记列表"，不要先走 [lark-vc](../lark-vc/GUIDE.md)，而应直接使用本 skill
> - 用户如果同时提到"会议 / 会 / 开会 / 某场会"，即使也提到了"妙记"，也应优先走 [lark-vc](../lark-vc/GUIDE.md) 先定位会议，再通过 [vc +recording](../lark-vc/references/lark-vc-recording.md) 获取 `minute_token`
...
- 纪要/逐字稿/总结/待办/章节内容获取 → [lark-vc](../lark-vc/GUIDE.md)（`vc +notes --minute-tokens`）
- 搜索历史会议记录 → [lark-vc](../lark-vc/GUIDE.md)
- 查询未来的会议日程 → [lark-calendar](../lark-calendar/GUIDE.md)
