...
后续 CLI 在 `data.cwd` 下执行；将 `data.workspace` 记为 `work_dir`、`data.draft_path` 记为 `draft_path`（已含工作区前缀）。草稿写入 `<data.cwd>/<draft_path>`，读取用 `--content "@./<draft_path>"`。
