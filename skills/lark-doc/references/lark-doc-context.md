...
| `<sheet>` / `<cite file-type="sheets">` | 提取 `token` 与 `sheet-id`，切到 `lark-sheets` 读取相关范围 |
| `<bitable>` / `<cite file-type="bitable">` | 提取 `token` 与 `table-id`，切到 `lark-base` 查询相关字段和记录 |
| `<synced_reference>` | 提取 `src-token` 与 `src-block-id`，只读取对应源 block |
