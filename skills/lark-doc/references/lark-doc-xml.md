...
禁止转义标签本身；只转义标签内部的文本内容。不同块级容器适用不同的转义规范：

### 1. 普通富文本内容 (`<p>`, `<h1>`-`<h6>`, `<td>`, `<th>`, `<li>`)
- **字符转义**：`<` → `&lt;`，`&` → `&amp;`，换行符 `
` → `<br/>`；`>` 无需强制转义（XML 规范在元素内容中原生支持）。
- **错误**：`&lt;p&gt;内容&lt;/p&gt;`
- **正确**：`<p>A &amp; B 的对比：1 &lt; 2</p>`

### 2. 纯文本代码块与行内代码 (`<pre><code>`, `<code>`) — 最小转义规范
- **原理**：飞书 Docx 服务端对代码块按 Raw String 处理，前端渲染时**不会执行二次 HTML 反转义**。
- **转义原则**：**仅转义 `&` (`&amp;`) 与 `<` (`&lt;`)**，**严禁将 `>` 转义为 `&gt;`**。
- **避坑说明**：若误将 `>` 转为 `&gt;`，代码中的箭头（如 `->`、`=>`、`──>`）会直接在前端字面显示为 `-&gt;`、`──&gt;`。
- **示例**：
  ```xml
  <!-- 正确写法 -->
  <pre lang="swift"><code>func process() -> Result&lt;Int, Error&gt; { ... }</code></pre>
  
  <!-- 错误写法（字面量泄露） -->
  <pre lang="swift"><code>func process() -&gt; Result&lt;Int, Error&gt; { ... }</code></pre>
  ```

### 3. 原生画板图表容器 (`<whiteboard type="mermaid|plantuml|svg">`) — DSL 原生隔离
- **原理**：画板内部为图表引擎（如 Mermaid、PlantUML）的编译器 DSL，服务端直接传入对应引擎进行矢量解析与渲染。
- **转义原则**：**严禁任何 HTML 实体转义**，必须保持 100% 原始 DSL 语法（如保持原生 `-->`、`---|` 等），否则会导致云端图表编译器解析报错。
