...
5. **画板按需使用**：流程、架构、路线图、指标趋势等可以用画板表达；只有当图示明显比文字更清楚时才插入
...
撰写或审查内容时，可以用下表判断某些信息是否适合图示。满足这些特征并不意味着必须画板化；只有当图示能明显减少解释成本、用户要求图表，或当前文档定位偏方案/汇报/展示时，才使用画板。
...
**判断规则：**
- 图示是可选增强，不是默认要求；短文档、纪要、草稿、纯说明、FAQ 等通常不需要画板
- 表格、分栏、列表和普通段落同样是一等表达方式，按信息密度和阅读场景选择
- 确定需要插入图表后，参照 [lark-doc-whiteboard.md](../lark-doc-whiteboard.md) 中的方式插入图表画板
