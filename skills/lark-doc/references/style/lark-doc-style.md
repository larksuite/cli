...
撰写或审查每个段落/章节时，判断内容是否适合用图表达。满足以下特征且图示能明显降低理解成本时，可使用画板；结构简单、文字更清楚的内容可以保持段落、列表或表格：
...
**判断规则：**
- 关键流程、架构、对比、风险链路等内容，如果纯文本会显著增加理解成本，可考虑画板
- 低重要度、局部辅助信息可用 `<table>` / `<grid>` / `<callout>` 或普通段落承载
- 确定需要插入图表后，参照 [lark-doc-whiteboard.md](../lark-doc-whiteboard.md) 中的方式插入画板。
