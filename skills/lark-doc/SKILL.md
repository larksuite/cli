...
- 用户需要统计文档的**总字数 / 总字符数**（word count / character count）时，使用 [`scripts/doc_word_stat.py`](scripts/doc_word_stat.py)：`lark-cli docs +fetch --doc "$URL" --doc-format xml --detail full --format json | python3 skills/lark-doc/scripts/doc_word_stat.py --protocol xml --lark-json --pretty`。
