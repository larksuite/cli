...
> - **飞书/Lark 云空间原生资源链接**：Docx 链接写入遵循 [`lark-shared`](../lark-shared/SKILL.md) 的「飞书资源链接写入」规则；构造 XML 时，能表达原生资源的标签或资源块（如 `<cite type="doc">`、`<sheet>`、bitable/base_ref 等）优先于泛用超链接。
