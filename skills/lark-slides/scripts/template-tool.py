    ...
    for raw_line in lines:
        ...
        if plain.startswith("页型索引"):
            ...
            for item in [part.strip() for part in ranges_raw.split("|") if part.strip()]:
                match = re.match(r"^(.+?)\s+([0-9,\-无]+)$", item)
                if not match:
                    ranges.append({"label": item, "range": "", "slide_numbers": []})
                    continue
                range_text = "" if match.group(2) == "无" else match.group(2)
                ranges.append(
                    {
...
def resolve_template_entry(index_data: dict[str, Any], template_selector: str) -> dict[str, Any]:
    ...
    normalized = re.sub(r"\.xml$", "", template_selector)
    for entry in index_data["templates"]:
        if entry["template_id"] == normalized or f'{entry["template_id"]}.xml' == f"{normalized}.xml":
            return entry

    as_path = Path(template_selector)
    if not as_path.is_absolute():
        as_path = (Path.cwd() / as_path).resolve()
    if not as_path.exists():
        fail(f"template not found: {template_selector}")
    filename = as_path.name
    for entry in index_data["templates"]:
        if f'{entry["template_id"]}.xml' == filename:
            return entry
    fail(f"template exists but is not indexed: {template_selector}")
...
def summarize_selection(index_data: dict[str, Any], template_selector: str, options: dict[str, Any]) -> dict[str, Any]:
    entry = resolve_template_entry(index_data, template_selector)
    selection = resolve_range_selection(entry, options)
    xml_info = parse_template_xml(get_template_path(entry))
    slide_summaries = [
