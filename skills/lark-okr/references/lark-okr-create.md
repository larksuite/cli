...
推荐大多数创建场景使用 `simple` 风格。格式为 `SemiPlainContent` JSON：
...
- `simple` 风格**不支持** `docs` 和 `images` 字段；如需插入文档链接或图片，请改用 `--style richtext`
...
- [ContentBlock 格式](lark-okr-contentblock.md) -- content 字段的两种输入风格
