...
**Avoid `kill -9` on consume processes**: some EventKeys with a **PreConsume hook** register server-side subscriptions via OAPI and may also register best-effort cleanup. `kill -9` skips process cleanup and can leave local state inconsistent. Prefer SIGTERM or closing stdin, and check the domain reference for whether that EventKey unsubscribes on exit.
