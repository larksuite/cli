...
|------|------------------|------|------------------|------|----------|
| 退货政策.docx | 退货售后 (wikcn_A) | 知识页 | import_docx | 通过 | 退货政策知识页，迁为 wikcn_A 子页（全文见下） |
| 配送时效.pdf | 物流配送 (wikcn_B) | 知识页 | node_create_docx | 收紧为进行中：仅部分解析 | 在 wikcn_B 下建承载子页，写配送时效正文（标页码，全文见下） |
| 退货政策原件.pdf | 退货售后 (wikcn_A) | 来源附件 | drive_upload | 通过（附件，不计页面） | 原文件留证 |

<逐份展开将写入的精确正文与治理表；import_docx 标明整理要点并迁为目标节点子页，node_create_docx 标明先建承载子页再写正文（标来源页码），均不写入目标分类节点本体>
