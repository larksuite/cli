...
> **认证与确认：** 普通搜索直接执行本页命令。用户明确要求检查登录态或当前身份，或命令返回认证、授权、scope、权限或 `1061005 auth failed` 时，再阅读 [`../lark-shared/SKILL.md`](../../lark-shared/SKILL.md)。
...
>
> **按完整标题定位：** 用户明确给出标题时使用 `--only-title`。标题不超过 30 个字符时直接作为 query；超长标题先选取不超过限制的稳定片段召回候选，再使用返回的标题字段判断完整标题是否严格相等。保持同一 query 和过滤条件按 `page_token` 最多检查 3 页；只有已检查全部结果且 `has_more=false`、跨页恰好一个严格匹配时才能继续写操作。达到 3 页仍有 `has_more=true`、没有严格匹配或存在多个严格匹配时停止写操作，请用户缩小范围或补充信息。`drive files list` 只用于枚举已知文件夹的直接子项，不替代标题搜索。
>
