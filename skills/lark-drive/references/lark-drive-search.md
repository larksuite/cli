...
>
> **按完整标题定位：** 用户明确给出标题时使用 `--only-title`。标题不超过 30 个字符时直接作为 query；超长标题先选取不超过限制的稳定片段召回候选，再使用返回的标题字段判断完整标题是否严格相等。仅有一个严格匹配时才能继续写操作；没有严格匹配时报告未找到，存在多个严格匹配时列出候选并请用户补充信息。`drive files list` 只用于枚举已知文件夹的直接子项，不替代标题搜索。
>
