...
用户明确指定知识空间、文件夹、群聊、人员、时间或资源类型时，将这些条件视为任务范围约束，而不是可自动放宽的搜索提示：

- 扩展 `query` 仅调整或增加关键词；始终保留用户指定的 `--space-ids`、`--folder-tokens`、`--chat-ids`、人员、时间和类型等过滤条件。
- 不要为了提高召回率自行删除、替换或放宽范围过滤。扩大数据来源属于任务范围变更，先取得用户明确同意。
- 限定范围内没有结果是合法结果；如实报告未找到，不要使用范围外结果补全答案。
- 只将能够验证属于指定范围的结果作为证据；来源无法验证时说明不确定性。
...
- **证据核验**：列表/统计类答案必须来自搜索结果中的实际 URL/token 和类型/时间字段；内容问答必须能指出使用了哪些非污染候选。没有可验证候选时，在保持所有范围过滤条件不变的前提下扩大 query 或翻页；仍无结果时如实报告未找到，不要直接编总结。
- **时间表达**：
