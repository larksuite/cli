...
>
> **标题词 + 正文词联合搜索**：如果用户同时给出标题关键词和正文关键词，并要求同一资源同时满足两项条件，先执行一条普通联合搜索：`lark-cli drive +search --query "标题词 正文词"`，再叠加 `--folder-tokens`、`--doc-types` 等作用域过滤。不要把这种联合搜索拆成“标题搜索 + 正文搜索”后自行拼交集；也不要把 `--only-title` 或 `intitle:` 用作主候选路径。只有用户明确只查标题时，才使用 `--only-title` 或 `intitle:`。
>
