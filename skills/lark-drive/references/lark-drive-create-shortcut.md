...
# 省略 --folder-token，默认创建到 root folder
lark-cli drive +create-shortcut \
  --file-token <DOCX_TOKEN> \
  --type docx
```
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--folder-token` | 否 | 目标父文件夹 token；省略时 CLI 会先解析当前身份的 root folder，并将快捷方式创建在根目录下 |
| `--file-token` | 是 | 源文件 token，表示被引用的原始文件 |
| `--type` | 是 | 源文件类型，推荐值：`file`、`docx`、`doc`、`sheet`、`bitable`、`mindnote`、`slides` |

## 输入规则

- 该 shortcut 的最小输入是 `--file-token` + `--type`
- 如果省略 `--folder-token`，CLI 会先调用 root folder 查询接口，拿到当前身份的根目录 token 后再创建快捷方式
...
- 省略 `--folder-token` 时，会先调用 `GET /open-apis/drive/explorer/v2/root_folder/meta` 获取 root folder token，再调用创建接口
