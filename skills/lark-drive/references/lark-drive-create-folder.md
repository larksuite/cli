# drive +create-folder（创建云空间（云盘）文件夹）

> **前置条件：** 先阅读 [`../lark-shared/SKILL.md`](../../lark-shared/SKILL.md) 了解认证、全局参数和安全规则。

在飞书云空间（云盘）中创建一个新文件夹。该 shortcut 对原生 `drive files create_folder` 做了一层更适合日常使用的封装：`--folder-token` 可省略，此时会在调用者根目录创建；如果使用 `--as bot`，创建成功后 CLI 会尝试把新文件夹的可管理权限自动授予当前 CLI 用户。
...
- 用户说“在云空间（云盘）新建一个文件夹 / 目录”时，优先使用 `drive +create-folder`
...
- [lark-drive](../SKILL.md) -- 云空间（云盘）全部命令
