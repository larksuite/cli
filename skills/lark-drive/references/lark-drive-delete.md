...
删除云空间（云盘/云存储）内的文件、文件夹、在线文档、Base 或 BaseApp。删除后资源会进入回收站。
...
# 删除 Base 本体；底层 Drive type 是 bitable
lark-cli drive +delete \
  --file-token <BASE_TOKEN> \
  --type bitable \
  --yes

# 删除 BaseApp 应用本体；传 app_token，shortcut 会把 baseapp 归一为 bitable
lark-cli drive +delete \
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--file-token` | 是 | 需要删除的资源 token；删除 BaseApp 时传 `app_token` |
| `--type` | 是 | 文件类型，可选值：`file`、`docx`、`bitable`、`base`、`baseapp`、`doc`、`sheet`、`mindnote`、`folder`、`shortcut`、`slides`。`base` / `baseapp` 是 shortcut 别名，底层请求都会发送 `type=bitable` |
| `--yes` | 是 | 确认执行高风险删除操作 |

## 行为说明

- **删除可能需要等待**：删除操作在服务端可能异步处理，shortcut 会在本次命令内自动做有限次数的结果轮询
- **BaseApp 与 Base 同路由**：删除 BaseApp 时传 `app_token`；shortcut 会把 `--type baseapp` 归一为 Drive `type=bitable`，和删除 Base 本体完全一致。直接调用原生 Drive files delete 时传 `type=bitable`
