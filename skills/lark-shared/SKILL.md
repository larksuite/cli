...
## 链接书写规则（飞书/Lark 资源 URL）

在富文本内容面（文档 Markdown/XML、电子表格单元格、Base 文本/描述字段等）写入链接时，按以下规则选择写法：

**默认写裸 URL**：飞书/Lark 云空间原生资源的链接，直接写原始 URL 字符串，**不要**默认包装成 `[标题](url)` 这类带显示文本的链接。裸 URL 能让编辑器保留转换为卡片、预览或 @文档 等原生形态的能力；包装成文字链接后会固定为普通超链接，丢失这些转换机会。

云空间原生资源包括：Docx 文档、电子表格（Sheets）、多维表格（Base/bitable）、知识库（Wiki）、云盘文件/文件夹（Drive）、幻灯片（Slides）、妙记（Minutes）的 URL。

**例外（可以使用带显示文本的链接）**：

- 用户明确要求自定义显示文本
- 外部（非飞书/Lark）URL
- 非原生资源页面的 URL，如分享落地页、下载直链、回调地址等
- 结构化 API 字段本身要求 link/rich_text 对象时，按该 API 的 schema 构造（如 Sheets 单元格的 `{"type":"url",...}`、IM 富文本的 `a` 标签）
