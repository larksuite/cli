...
所有 Base 操作优先显式使用 `--as user`。仅在用户明确要求使用应用身份，或 user 身份确认对目标资源无访问权限且错误没有用户授权恢复路径时，对支持 bot 的命令尝试一次 `--as bot`。

- user 身份出现授权提示或 `missing_scopes` 时，先转 `lark-shared` 恢复用户授权，再以 `--as user` 重试，不直接切换 bot。
- 资源级无权限且没有授权恢复提示时，可以用 `--as bot` 重试一次；bot 仍失败即停止并报告权限边界。
- 遇到 `91403` 或明确不可访问错误时，不在 user / bot 之间循环切换；每种适用身份至多尝试一次。
