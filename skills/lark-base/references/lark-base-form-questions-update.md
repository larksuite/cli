...
- 更新后再次执行 `+form-questions-list`。按同一 `id` 比较本轮未获授权变化的全部配置，至少包括 `title`、`description`、`required`、`option_display_mode` 和 `visible_rule`。任一字段意外变化时，使用写前保存的**完整题目配置**恢复（不能只传 `id` / `title`，否则其他字段会再次按默认值重建），恢复后重新回读；通过前不得继续提交表单或宣称完成。
