...
`+workflow-get` 返回工作流 API 的表示，不保证与原生编辑器中的全部配置无损往返。已报告的 1.0.95 情况包括：编辑器中存在的内嵌公式在写入值中读成 `value_type: text` 的空字符串、分支左值读成 `null`、循环输入读成空数组。该观察不表示所有空值都是错误，也未确定所有版本的影响范围；见 [问题 #2754](https://github.com/larksuite/cli/issues/2754)。

审计或调用 `+workflow-update` 替换已有步骤前：

1. 对照原生编辑器中已保存的公式、条件操作数、循环输入及引用，核实导出的完整性。仅凭导出中的空值，不应判定为空写入、恒假分支或无引用步骤。
2. 区分数据表公式字段与工作流内嵌公式；前者读取完整不代表后者也完整。直接通过 `api GET` 调用同一端点也不保证能恢复缺失表达式。
3. `+workflow-update` 是完整替换，省略 `steps` 会清空原有步骤。不要将已发现缺失的导出当成备份或修改后整份回写；仅把所有返回字段原样保留仍不足以保护未被返回的配置。
4. 若导出与原生配置不一致，暂停该次替换，继续使用原生编辑器，或等待上游支持完整表示。保留脱敏差异供排查，不猜测公式、不用空值填补缺失内容。

这些限制针对读取保真度，不改变合法空值的语义，也不表示仅凭 CLI 输出就能验证工作流执行成功。
