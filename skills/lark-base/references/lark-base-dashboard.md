...
> [!IMPORTANT]
> - **四个 key 必须齐全**：缺哪个服务端就把哪个按 0 处理（只传 `{"x":6}` 会把组件压成零尺寸，不是"只挪不改大小"）。本地会拒绝残缺对象。
> - 坐标**取值不做校验**：越界、负值、重叠都原样透传，服务端也不做碰撞检测，需要不重叠布局时由调用方自行规划。
> - 不传 `--position`：create 由服务端自动装箱，update 保持当前布局不变。
> - **坐标只能写、不能读**：`+dashboard-block-get` / `+dashboard-block-list` 本期不返回 `x/y/w/h`。所以无法"读取源看板布局再复刻"，坐标需要调用方自己规划并记录。
> - 想要精确落点用 `--position`；只想一键美化、不在意具体位置用 `+dashboard-arrange`（服务端整盘智能重排，无法指定坐标）。两者用途不同，别互相替代，也别在用户要求精确位置时用重排糊弄过去。
