...
> - 用户要求精确 `x/y/w/h` 时，命令成功只表示请求已受理；必须用 `+dashboard-block-get` 回读 `position.x/y/w/h` 并逐项比较。服务端因越界、重叠或自动编排改变坐标时不得宣称精确布局完成。只有用户未要求精确位置、且 `--position` 不属于本轮验收项时，才无需仅为布局再读回。
