...
1. **范围与粒度：** 写明分析对象、筛选范围、目标粒度和分组维度。用户指定的字段或公式原样优先；存在多个候选日期或分类字段时，先确定主分组维度组合并说明选择依据，其他候选只能作为清楚标注的补充。若不同选择会实质改变结论，且没有用户要求、直接定义该口径的字段公式、经用户确认的字段定义或已确认业务契约可用于消歧，先询问用户；字段名、类型或枚举的存在以及业务惯例本身不构成消歧依据。
2. **度量与比率：** 写明直接度量字段、聚合方式和单位；比率必须写成 `分子 / 分母`，不得静默从分母排除仍属于用户所述分析对象的记录。更窄的适用集合只有在用户明确要求、用户明确指定某个 View 为任务范围、字段公式或经用户确认的字段定义直接编码该指标，或已确认业务契约直接定义该指标时，才可作为主口径；字段、枚举或 View 的存在以及业务惯例本身不构成授权。否则只可作为替代口径单列，不能覆盖主口径。
