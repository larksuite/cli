...
3. **记录**：用 `+record-list --view-id <view_id>` 读取代表性命中项；只有意图集合存在排除范围时，才从原表检查至少一条应排除项。目标集合按定义没有排除项时记录该事实，不为凑负样本伪造条件。结果必须符合名称和用户意图。
...
- 日期条件稳定写法用 `ExactDate(...)` 或 `Today` / `Yesterday` / `Tomorrow`；其他相对范围按下方动态 Formula fallback 处理。
- `formula` / `lookup` 的 value 形状不固定；拿不准时先读当前 filter 或字段定义，或根据错误提示修正类型。

### 持久视图的相对时间

用户要求“本月、今年、年度、年底、近 N 天/月、超过 N 天/月、即将到期”等会随时间推进的范围时，不能把运行当天换算成固定 `ExactDate(...)` 边界后保存到 View。tuple filter 当前明确支持 `Today`、`Yesterday`、`Tomorrow`；其他相对范围只有在当前 CLI / 服务端实测接受并能原样回读时才直接使用。未确认、校验失败或回读漂移时，创建使用 `TODAY()` / `YEAR(TODAY())` 等动态函数的辅助 Formula，再让 View 筛选该字段。
