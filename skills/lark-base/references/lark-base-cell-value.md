...
用 JSON number，不要用带单位或千分位的字符串。货币、百分比、评分等数字类字段也按数字写入，展示格式由字段配置决定。
...
单选/阶段用选项名字符串；多选用选项名数组。选项名必须与字段配置一致，不要传自然语言近义词。
...
用对象数组，元素至少包含 `id`。`id` 必须是可被当前 Base 识别的用户 ID；写入前确认字段是否允许多选人员。
...
用对象数组，元素至少包含关联表中的 `record_id`。不要传记录标题；先用 `+record-list` / `+record-search` 找到目标记录 ID。
