...
无论新建字段题目还是复用已有字段题目，表单题目的**顶层 `type`** 都仅支持 `text`、`number`、`select`、`datetime`、`user`、`attachment`、`location` 这 7 种字段类型，未列出的顶层类型不支持。下表中的 `Url` / `Email` / `Barcode` 等名称指不能作为顶层 `type` 传入的别名，不否定合法的 `type:"text"` + `style.type:"url"/"email"/"barcode"`；后者仍按下方 style 规则支持。
...
6. 用户枚举了要收集的信息项时，把每一项逐项与写后 `+form-questions-list` 按题目 ID/标题配对；缺项必须补齐。信息项已经是表内受支持字段时用 `use_existing_field:true` 加入，不能因为字段存在就把它误判成表单已有题目。
