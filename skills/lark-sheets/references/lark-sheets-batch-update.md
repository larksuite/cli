...
**`+dropdown-update` 的选项模式（`--options` / `--source-range` 二选一）+ 配色规则**（默认不传 `--colors`、使用内置 10 色色板；仅在用户明确要求或选项有语义配色时传浅色低饱和度颜色）见 [`lark-sheets-write-cells`](./lark-sheets-write-cells.md) 的「Dropdown 选项 + 配色」节，本文不重复。`+dropdown-delete` 不涉及这些 flag。
