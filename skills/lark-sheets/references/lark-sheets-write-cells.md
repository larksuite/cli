...
3. **回读抽样校验**：写完关键值 / 普通公式后，用 `+csv-get` 或 `+cells-get` 重新读取写入区域，至少抽样 3-5 个代表性单元格（首 / 中 / 末），核对值与预期一致（与本地脚本计算的预期值对照）。AI 公式的计算状态不要先用 `+cells-get` 轮询，直接按 `lark-sheets-formula-verify` 的 `+formula-verify --ai-only --range` 异步抽检规则处理。
...
1. **普通公式写完立即读回**：`+cells-set` 后紧跟 `+csv-get`（或 `+cells-get`）读目标范围前几行，检查是否出现 `#VALUE!` / `#NAME?` / `#REF!` / `#N/A` / `#DIV/0!` / `#NUM!`；AI 公式跳过这一步，第一校验入口是 `+formula-verify --ai-only --range`（禁止先用 `+cells-get` / `+csv-get` 轮询 AI 结果）
...
⚠️ **公式写入的默认收尾不是停在回读，而是继续跑 `+formula-verify`**：`+csv-get` / `+cells-get` 的抽样回读覆盖不到整列中段、隐藏行、被条件格式遮蔽的错误，也看不到 `partial` 截断。普通公式必须跑到 `status='success'`；AI 公式必须先用 `+formula-verify --ai-only --range <代表性范围>` 抽检，禁止先用 `+cells-get` / `+csv-get` 轮询，没有明确失败 / 不支持状态时允许带 pending 交付并告知用户后台仍在计算。
