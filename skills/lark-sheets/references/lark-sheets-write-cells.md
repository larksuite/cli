...
带显示文本的超链接、@人、@文档这类富内容**必须**走 `+cells-set` 的 `rich_text` 字段（`cells[].rich_text` 数组，每段一个对象、带 `type`），**不能**直接传普通字符串——纯字符串只会被当作纯文本存进单元格。完整字段跑 `lark-cli sheets +cells-set --print-schema --flag-name cells`。

飞书/Lark Drive 原生资源链接遵循 [`lark-shared`](../../lark-shared/SKILL.md) 的「飞书资源链接写入」规则；需要 `rich_text` link 或自定义显示文本时按共享规则和目标单元格类型处理。
