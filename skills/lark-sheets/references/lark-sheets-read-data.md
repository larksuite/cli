...
| `--include` | string_slice | optional | 要返回的信息类别，逗号分隔多个。`raw_value` 返回单元格原始值或公式计算结果原值，保留 number / string 类型，与 `formula` 互斥。`truncation` 会额外按行高列宽 / 字号 / 自动换行估算每个单元格是否被截断显示，返回 `isRowTruncated` / `isColTruncated`（有额外计算开销，仅排版检查 / 调整行高列宽前才开）（可选值：`value` / `formula` / `raw_value` / `style` / `comment` / `data_validation` / `conditional_format` / `truncation`） |
