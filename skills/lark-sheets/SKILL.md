...
2. **真实写回 + 回读校验**：交付必须是对在线表格的真实写入，写完用 `+csv-get` / `+cells-get` / `+<对象>-list` 回读确认生效（顺带确认无截断 / 溢出 / 科学计数法）——**返回 `ok` 只代表请求被接受，不代表结果符合预期**。**图表最终交付门禁：最后一次修图后按 `lark-sheets-chart` 运行统一质检，读完其 `read_required[].path` 全部图片并分别确认数据语义、静态质量、视觉结果；覆盖不全、图片不可验、任一图片存在可见重叠 / 截断 / 遮挡 / 无法逐项辨认，或修图后未重跑时都不得声称完成，且不得以“核心信息可读”“只是小问题”、静态质检或 `lark_sheet_selfcheck.py` 的通过信号放行。**迭代中的轻量单图验证见 `lark-sheets-chart`。回读值可能带「值(样式)」注记（如 `49.6(V-Align: bottom)`），据此回写前先剥离注记只留纯值；写公式后用 `+cells-get --include formula` 核对**真实落格**（仅看显示值不能证明联动）；筛选 / 排序后核对前几行，删除后确认已空。不要只在文本里声称"已完成"。
