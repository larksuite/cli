...
1. **上传文件至云空间（云盘）获取 file_token**
   - 使用 `lark-cli drive +upload` 命令上传本地文件到云空间（云盘）：
     ```bash
...
```bash
# 通过已上传到云空间（云盘）的 file_token 生成妙记
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--file-token <token>` | 是 | 已经上传到飞书云空间（云盘）的音视频文件的 file_token |
...
本接口不直接处理本地文件的上传，必须先使用 `drive +upload` 将文件上传到云空间（云盘）获取 `file_token`，然后再调用本接口。

### 2. 先上传，再生成妙记

推荐流程如下：

1. 使用 `lark-cli drive +upload --file <path>` 上传本地音视频文件到云空间（云盘）
...
- [drive +upload](../../lark-drive/references/lark-drive-upload.md) -- 上传文件到云空间（云盘）
