---
name: lark-whiteboard-draw
description: >
  当需要在飞书画板中绘制架构图、流程图、思维导图、时序图或其他可视化图表时使用此 skill。
...
Step 2: 生成完整 DSL（含颜色）
  ...
  - 推荐使用图标让图表更直观，运行 `npx -y @larksuite/whiteboard-cli@^0.1.0 --icons` 查看可用图标，选取合适的图标, 但不要过度使用或者所有图表都用图标, 根据图表类型和内容选择是否使用图标
...
**渲染**：
```bash
npx -y @larksuite/whiteboard-cli@^0.1.0 -i ./diagrams/2026-03-27T143000/diagram.json -o ./diagrams/2026-03-27T143000/diagram.png    # DSL
npx -y @larksuite/whiteboard-cli@^0.1.0 -i ./diagrams/2026-03-27T143000/diagram.mmd -o ./diagrams/2026-03-27T143000/diagram.png     # Mermaid
```
...
> ```bash
> npx -y @larksuite/whiteboard-cli@^0.1.0 --to openapi -i <输入文件> --format json | lark-cli whiteboard +update --whiteboard-token <Token> --overwrite --dry-run --as user
> ```
>
> **解析结果并拦截**
> - 仔细阅读 Dry Run 的输出日志。
...
```bash
npx -y @larksuite/whiteboard-cli@^0.1.0 --to openapi -i <输入文件> --format json | lark-cli whiteboard +update --whiteboard-token <画板Token> --yes --as user
```

> 画板一经上传不可修改。如需应用身份上传，将 `--as user` 替换为 `--as bot`。
> 如果画板非空，先加 `--overwrite --dry-run` 检查待删除节点数，向用户确认后去掉 `--dry-run` 执行。

lark-cli 中画板相关能力的详细说明参考 [lark-whiteboard SKILL](../lark-whiteboard/SKILL.md)
