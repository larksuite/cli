...
6. 命令默认输出结构化事件契约：`meeting`、`identity`、`current_participants`、`events`、`warnings`、`has_more`、`page_token`；`identity` 表示当前读取身份，参会人含 `participant_type`、`role`、`is_self` 和可读 `label`，事件细节保留在 `payload`。
