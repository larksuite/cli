...
name: lark-vc-agent
version: 1.0.0
description: "飞书视频会议 — 让机器人替你参加正在开的会议。给我一个 9 位会议号，就能代你入会、实时看到会议里谁加入、谁离开、谁发言、谁共享屏幕，结束后再退出。1. 用户说"帮我入会 123456789"、"加入会议"、"代我参会"、"让机器人进会旁听"时使用 +meeting-join（会真的让机器人加入会议，产生真实的参会记录）。2. 用户想**实时**知道"会议里发生了什么"、"谁来了"、"谁发言了"、"有人共享屏幕了吗"（会议**还在进行中**），先让机器人入会，再用 +meeting-events 读事件。3. 用户说"退出会议"、"让机器人离开"、"结束参会"时用 +meeting-leave。注意：机器人只能看它**自己参加过**的会议里的事件；已经开完的会议、只想看参会名单、查纪要或逐字稿，请用 lark-vc 技能。"
metadata:
  requires:
    bins: ["lark-cli"]
