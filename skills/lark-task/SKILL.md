...
> **描述格式化警告（必读）**：
> `--description` 和任务评论的 content 在飞书客户端按 Markdown 渲染，但解析失败的 span 会被**静默吞掉，不会降级为纯文本**——API 写入成功、回读不变，只有人看客户端才看到文字消失（如 QA 交接单里 15 个 `**bold**` 导致 5 个章节标题空白）。Agent 写任务描述**无法自行发现此失败**。
> 已知高危写法（基于 33 例实测）：
> 1. 成对的 `**` `__` `~~` `***`：无条件被吞；未闭合的 `**` 会吃掉直到下一个 `**` 的整段内容（含换行）。
> 2. 单个 `*` `_`：仅当闭合符后跟 ASCII 标点/空白/行尾时被吞（中文标点不触发）。会把 `slg_icon_xxx` 渲染成 `slgxxx`。
> 3. `<x>`：`<` 后跟 ASCII 字母/`/`/`!` 时被吞（HTML 标签启发式）。
> 4. 行首 `# ` 标题：整行消失（不是渲染成标题）。
> 5. `[label] (url)` 带空格：只留 label，括号部分消失（无空格形式 `[label](url)` 配非法 URL 会报 1470400）。
> **规避**：需要字面量时尽量不用 Markdown 特殊字符；无法避免时用 `--dry-run` 无法发现（不报错），只能人工在客户端确认。存在未决 issue #2359。
>
> > **查询注意**：
