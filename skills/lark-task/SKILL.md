...
> **默认搜索范围**：当用户提供了查询关键字但**没有明确指定查询主体**时（例如”搜索标题包含'测试'的任务”、”找一下'会议'相关的任务”，没有说”所有人的”或”团队的”或”某某的”），应默认将搜索范围限定为**当前用户自己的任务**。具体做法：优先使用 `+get-my-tasks --query “关键字”` 在用户负责的任务中搜索；如果需要更广的范围（例如包含用户创建、关注的任务），则使用 `+search` 并附加 `--assignee` / `--creator` / `--follower` 过滤为当前用户的 `open_id`。只有当用户**明确要求**搜索所有任务、全部任务、团队任务、或他人的任务时，才使用不带人员过滤的 `+search`。
