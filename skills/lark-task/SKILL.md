...
> **默认搜索范围**：当用户提供了查询关键字但**没有明确指定查询主体**时（例如”搜索标题包含'测试'的任务””找一下'会议'相关的任务”，没有说”所有人的”或”团队的”或”某某的”），应默认搜索**当前用户自己的任务**。具体做法：优先使用 `+get-my-tasks --query “关键字”` 在用户负责的任务中搜索（这是确定性路由选择，不是给 `+search` 加隐式过滤）。只有当用户**明确要求**搜索所有任务、团队任务、或他人的任务时，才使用 `+search`。`+search` 自身应保持与客户端一致的行为，不对结果做隐式的人员过滤。
