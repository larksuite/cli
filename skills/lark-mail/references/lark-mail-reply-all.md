...
列表型参数兼容重复 flag 与旧单 flag 写法，例如 `--to a@example.com --to b@example.com` 和 `--to 'a@example.com,b@example.com'`；`--remove`、`--attach` 同理。`--inline` 每次接受一个 JSON object 或 array，重复值按出现顺序合并且不会按逗号切分。
...
| `--to '<email>'` | 否 | 额外收件人。可重复传或单次传逗号地址列表；追加到自动聚合结果 |
| `--cc '<email>'` | 否 | 额外抄送。可重复传或单次传逗号地址列表 |
| `--bcc '<email>'` | 否 | 密送邮箱。可重复传或单次传逗号地址列表。与 `--event-*` 不兼容 |
| `--remove '<email>'` | 否 | 从自动聚合结果中排除的邮箱。可重复传或单次传逗号地址列表；按传入顺序处理 |
| `--plain-text` | 否 | 强制纯文本模式，忽略所有 HTML 自动检测。不可与 `--inline` 同时使用。纯文本模式下也会自动追加纯文本签名（HTML 签名经 `PlainTextFromHTML` 转换，内联图片丢弃） |
| `--attach '<path>'` | 否 | 附件文件路径。可重复传或单次传逗号路径列表；按传入顺序追加，单个文件上限 3 GB |
| `--inline '<json>'` | 否 | 手动指定内嵌图片 CID 映射。每次接受 JSON object 或 array；可重复传并按顺序合并，不按逗号切分。不可与 `--plain-text` 同时使用 |
