...
| `--to '<emails>'` | 否 | 额外收件人列表，可重复；每次可传一个地址或 RFC 逗号列表，按顺序追加到自动聚合结果 |
| `--cc '<emails>'` | 否 | 额外抄送列表，可重复；每次可传一个地址或逗号列表，按顺序合并 |
| `--bcc '<emails>'` | 否 | 密送列表，可重复；每次可传一个地址或逗号列表，按顺序合并。与 `--event-*` 不兼容 |
| `--remove '<emails>'` | 否 | 排除地址列表，可重复；每次可传一个地址或逗号列表，整体按输入顺序处理 |
...
| `--inline '<json>'` | 否 | 每次接受一个 JSON object 或 JSON array，可重复并稳定合并。`file_path` 必须是相对路径，CID 应唯一；不可与 `--plain-text` 同时使用 |
