...
| `--signature-id <id>` | 否 | 显式指定签名 ID。运行 `mail +signature` 查看可用签名；优先级高于自动默认签名。HTML 正文继续插入 HTML 签名，纯文本正文会追加纯文本签名 |
...
- 未显式传 `--signature-id` 且未传 `--no-signature` 时，会自动读取当前 sender 对应的发送默认签名；`--from` alias 会按实际 sender 地址精确匹配签名 usage。
