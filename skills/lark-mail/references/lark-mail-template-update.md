...
| `--attach '<paths>'` | 追加非 inline 附件，不替换已有附件。可重复；每次可传一个相对路径或逗号列表，按整体输入顺序上传 |
| `--inline '<json>'` | 追加 inline 图片，不替换已有附件。每次接受一个 JSON object 或 JSON array，可重复并稳定合并；`file_path` 必须是相对路径，CID 应唯一；最终模板为纯文本模式时会被拒绝 |
