...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--to '<emails>'` | 否 | 收件人列表，可重复；每次可传一个地址或兼容 RFC display-name 的逗号列表，按输入顺序合并。省略时草稿不带收件人 |
| `--subject <text>` | 是 | 草稿主题 |
...
| `--cc '<emails>'` | 否 | 抄送列表，可重复；每次可传一个地址或逗号列表，按输入顺序合并 |
| `--bcc '<emails>'` | 否 | 密送列表，可重复；每次可传一个地址或逗号列表，按输入顺序合并。与 `--event-*` 不兼容 |
