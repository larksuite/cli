...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--to '<email>'` | 否 | 完整收件人列表。可重复传，也可在一次参数值内传逗号分隔的地址列表。支持 `Alice <alice@example.com>` 和带逗号的 quoted display-name。省略时草稿不带收件人 |
| `--subject <text>` | 是 | 草稿主题 |
...
| `--cc '<email>'` | 否 | 完整抄送列表。可重复传，也可在一次参数值内传逗号分隔的地址列表 |
| `--bcc '<email>'` | 否 | 完整密送列表。可重复传，也可在一次参数值内传逗号分隔的地址列表。与 `--event-*` 不兼容（见 `+send` 日程邀请约束） |
