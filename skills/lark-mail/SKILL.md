...
| [`+watch`](references/lark-mail-watch.md) | Watch for incoming mail events via WebSocket (requires scope mail:event and bot event mail.user_mailbox.event.message_received_v1 added). Run with --print-output-schema to see per-format field reference before parsing output. **Use `+watch` when** you want subject/body/attachments inline in the stream (auto-fetches via `messages.batch_get` per event) **or** need `--folders`/`--labels` filtering. **Use the unified entry** `lark-cli event consume mail.user_mailbox.event.message_received_v1 --as user` when you only need the new-mail signal (emits `message_id` only) and want the standard `lark event` contract (`--max-events`/`--timeout`/ready-marker); follow up with `mail +message --message-id <id>` for full content. |
