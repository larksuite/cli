...
用户级黑白名单通过 `user_mailbox.allow_sender` 和 `user_mailbox.blocked_sender` 两个原生 API 资源管理；租户级 `allowed_senders` / `blocked_senders` 是管理员名单，不要混用。首次使用先申请 scope：

```bash
lark-cli auth login --scope "mail:user_mailbox.message:readonly mail:user_mailbox.message:modify"
```

调用时使用 `--as user`，`user_mailbox_id` 通常传 `"me"`。读取用 `list`，`keyword` 为空表示列出全部，非空表示搜索：
...
设置和删除都走批量接口；`sender_type` 为 `1` 表示邮箱地址，`2` 表示域名：
...
lark-cli mail user_mailbox.blocked_sender batch_create --as user \
  --params '{"user_mailbox_id":"me"}' \
  --data '{"items":[{"sender":"example.com","sender_type":2}]}'

lark-cli mail user_mailbox.blocked_sender batch_remove --as user \
  --params '{"user_mailbox_id":"me"}' \
  --data '{"senders":["example.com"]}'
```
