...
| [`+thread-modify`](references/lark-mail-thread-modify.md) | Modify entire mail threads with one batch request; accepts repeatable `--thread-id`, label ID, and folder ID flags and passes through API data. |
| [`+thread-trash`](references/lark-mail-thread-trash.md) | Soft-delete entire mail threads with one batch request; accepts repeatable `--thread-id`, passes through API data, and requires `--yes`. |
