...
### user_mailbox.threads
  ...
  - `list` — 通过指定文件夹或标签，列出对应位置下的邮件会话列表。接口可返回邮件会话ID和会话下最新一封邮件的摘要。folder_id 和 label_id 必须且只能提供一个。Selector rule: `folder_id` and `label_id` are mutually exclusive and form a cross-field exactly-one (XOR) constraint; neither parameter is independently required. Truth table: neither provided -> invalid local CLI validation error; both provided -> invalid local CLI validation error; folder only -> valid; label only -> valid. Valid examples: `lark-cli mail user_mailbox.threads list --user-mailbox-id me --page-size 20 --folder-id INBOX` and `lark-cli mail user_mailbox.threads list --user-mailbox-id me --page-size 20 --label-id FLAGGED`.
