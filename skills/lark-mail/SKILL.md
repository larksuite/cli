...
### user_mailbox.allow_senders
  ...
  - `batch_create` — 批量将发件人加入指定用户邮箱的「信任发件人」白名单。支持按邮箱地址 (sender_type=1) 或域名 (sender_type=2) 添加。单次最多 100 项，单用户黑白名单合计最多 2000 项；与黑名单互斥（添加白名单会从黑名单删除对侧记录）。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--data '{"items":[{"sender":"trusted@example.com","sender_type":1}]}'`.
  - `batch_remove` — 批量从指定用户邮箱的「信任发件人」白名单中删除发件人。senders 中每项可以是邮箱地址或域名（与添加时一致）。批量删除按字面值哈希匹配，可兼容历史大写数据。单次最多 100 项。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--data '{"senders":["trusted@example.com"]}'`.
  - `list` — 列表/搜索指定用户邮箱的「信任发件人」白名单。支持按发件人地址或域名前缀搜索 (keyword)。返回列表按创建时间倒序，使用 page_token + page_size 进行分页。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--params user_mailbox_id=me --params page_size=50`. If error code `15180304` is returned, retry later and retain the `request_id` when reporting persistent failures.

### user_mailbox.blocked_senders

  - `batch_create` — 批量将发件人加入指定用户邮箱的「屏蔽发件人」黑名单。支持按邮箱地址 (sender_type=1) 或域名 (sender_type=2) 添加。单次最多 100 项，单用户黑白名单合计最多 2000 项；与白名单互斥（添加黑名单会从白名单删除对侧记录）。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--data '{"items":[{"sender":"spam@example.com","sender_type":1}]}'`.
  - `batch_remove` — 批量从指定用户邮箱的「屏蔽发件人」黑名单中删除发件人。senders 中每项可以是邮箱地址或域名（与添加时一致）。批量删除按字面值哈希匹配，可兼容历史大写数据。单次最多 100 项。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--data '{"senders":["spam@example.com"]}'`.
  - `list` — 列表/搜索指定用户邮箱的「屏蔽发件人」黑名单。支持按发件人地址或域名前缀搜索 (keyword)。返回列表按创建时间倒序，使用 page_token + page_size 进行分页。Identity: Meta supports `user` and `tenant`; use `--as user` with `user_mailbox_id=me` (tenant identity requires an explicit email address or open_id). Example: `--params user_mailbox_id=me --params page_size=50`. If error code `15180304` is returned, retry later and retain the `request_id` when reporting persistent failures.
...
| `user_mailbox.allow_senders.list` | `mail:user_mailbox.message:readonly` 或 `mail:user_mailbox.message:modify` |
...
| `user_mailbox.blocked_senders.list` | `mail:user_mailbox.message:readonly` 或 `mail:user_mailbox.message:modify` |
