...
邮箱是用户的个人资源，**默认使用 `--as user`（用户身份）请求**。

- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。需要先 `lark-cli auth login --domain mail` 完成用户授权。
- **`--as bot`**：以应用身份访问邮箱。需要在飞书开发者后台为应用开通相应权限，否则请求会被拒绝。

1. 操作用户自己的邮箱 → 使用 `--as user`，未登录时先使用 `lark-cli auth login --domain mail` 进行登录
2. 应用级批量操作（如管理员代操作） → 使用 `--as bot`，确保应用已开通对应权限
