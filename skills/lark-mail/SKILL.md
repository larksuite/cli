...
**Warning**：`+send` / `+draft-create` / `+reply` / `+reply-all` / `+forward` + 模板且模板自带 tos/ccs/bccs 时，CLI 在 stderr 打印：`warning: template to/cc/bcc are appended without de-duplication; you may see repeated recipients. Review recipients before sending, or run +template-update to clear template addresses.`
...
### 用户黑白名单：个人邮箱级 allow/block senders

用户级黑白名单使用 `user_mailbox.allow_sender` 和 `user_mailbox.blocked_sender`。它不同于租户级 `allowed_senders` / `blocked_senders`：用户级名单只管理当前授权邮箱的个人 allow/block 记录，通常 `user_mailbox_id` 传 `"me"`；租户级名单影响整个租户，不要混用。
...
### user_mailbox.allow_sender
  ...
  - `batch_create` — 批量加入用户白名单发件人。添加自身地址或本租户域名时，后端会返回明确错误信息；请调整 sender 后重试。
  - `list` — 列出或按关键字搜索用户白名单发件人。搜索返回 456/UserAllowBlockCachePreparing 表示搜索缓存准备中，请稍后重试；也可以先执行不带 keyword 的 list 触发缓存初始化。
  - `batch_remove` — 批量删除用户白名单发件人。

### user_mailbox.blocked_sender

  - `batch_create` — 批量加入用户黑名单发件人。添加自身地址或本租户域名时，后端会返回明确错误信息；请调整 sender 后重试。
  - `list` — 列出或按关键字搜索用户黑名单发件人。搜索返回 456/UserAllowBlockCachePreparing 表示搜索缓存准备中，请稍后重试；也可以先执行不带 keyword 的 list 触发缓存初始化。
  - `batch_remove` — 批量删除用户黑名单发件人。
