...
| [`+draft-send`](references/lark-mail-draft-send.md) | Send one or more existing mail drafts sequentially (requires --yes). Per-draft failures are isolated and aggregated. Optional --send-separately true/false overrides each draft's separate-send setting first (save-then-send; a draft whose save fails is not sent); omitted, each draft is sent with its server-stored setting. |
