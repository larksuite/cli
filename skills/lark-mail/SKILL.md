...
- **用户级发件人白名单 / 黑名单（User allow/block senders）**：当前用户个人邮箱的允许/屏蔽发件人名单，通过 `user_mailbox.allow_senders` 与 `user_mailbox.blocked_senders` 管理。它们只影响当前用户，与租户级 `allowed_senders` / `blocked_senders` 不是同一套能力。
...
3. 用户级发件人白名单 / 黑名单仅支持 user token → 必须显式使用 `--as user`。读取需 `mail:user_mailbox:readonly`，写入需 `mail:user_mailbox`；scope 不足时重新 `lark-cli auth login --domain mail --scope "<missing_scope>"`。
...
使用场景：用户要求“把某个发件人加入我的白名单/黑名单”“屏蔽这个发件人”“查看我自己的允许发件人/屏蔽发件人名单”时，使用本节命令。所有命令只支持 `--as user` 和 `user_mailbox_id=me`，不要尝试 `--as bot` 或代操作其他邮箱。

**不要与租户级能力混用**：

- `user_mailbox.allow_senders` / `user_mailbox.blocked_senders`：用户个人名单，影响当前用户，当前 Skill 管理的是这两类资源。
- 租户级 `allowed_senders` / `blocked_senders`：管理员/租户范围名单，作用于整个租户，不用于“我的白名单/黑名单”需求。

调用前先看 schema，确认当前二进制的参数结构：
...
lark-cli schema mail.user_mailbox.blocked_senders.list
lark-cli schema mail.user_mailbox.blocked_senders.batch_create
lark-cli schema mail.user_mailbox.blocked_senders.batch_remove
```
...
1. `batch_create` 的 `items[].sender` 可填邮箱地址或域名；`sender_type` 默认用 `1`，表示 sender 来源。
2. `batch_remove` 的 `senders` 只放待删除的邮箱地址或域名。
3. 加入黑名单或白名单前，先向用户展示目标 sender 列表和名单类型并取得确认。
4. 业务会保持黑白互斥、自地址/自域名保护和配额限制；服务端返回的 code/message 要原样告知用户。

搜索缓存提示：

- `list` 带 `keyword` 时会走搜索缓存。若返回 HTTP 456 或 message 表示 cache empty / 搜索缓存未就绪，提示用户稍后重试。
- 遇到缓存为空时，可先不带 `keyword` 列表一次触发缓存重建，再等待片刻重试 keyword 查询。
...
### user_mailbox.allow_senders
  ...
  - `list` — 分页列出或按 keyword 查询当前用户白名单发件人。
  - `batch_create` — 批量将邮箱地址或域名加入当前用户白名单。
  - `batch_remove` — 批量从当前用户白名单删除邮箱地址或域名。

### user_mailbox.blocked_senders

  - `list` — 分页列出或按 keyword 查询当前用户黑名单发件人。
  - `batch_create` — 批量将邮箱地址或域名加入当前用户黑名单。
  - `batch_remove` — 批量从当前用户黑名单删除邮箱地址或域名。
