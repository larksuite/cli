...
- `+release-get` 返回 `current_node_info.current_status=PENDING` 时立即加载 [`lark-apps-release-get.md`](references/lark-apps-release-get.md)，停止轮询并告知当前用户正在等待审批负责人处理；不得假定当前用户或 `submitted_by` 是审批人。
