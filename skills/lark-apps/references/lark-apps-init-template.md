...
用户要在本地开发一个 Web 应用（纯前端或全栈）并计划后续部署到妙搭时，用它初始化技术栈模板。它不创建妙搭应用、不打任何 Lark API、不涉及 git/沙箱（唯一的远端交互是对 npm registry 的只读下载）；只在本地 scaffold 项目。已有项目目录时不要用它（目标目录必须为空或不存在）。
