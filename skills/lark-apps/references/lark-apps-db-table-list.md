...
- 成功读取 `data.items[]`；每项字段是 `name`、`description`、`estimated_row_count`、`size_bytes`、`column_count`（列数）。CLI 默认不透出每表完整 `columns[]`（与 `+db-table-schema` 重复且放大 token），只给 `column_count`；要完整列定义/索引/约束用 `+db-table-schema`。
