...
交互式 `lark-cli auth login`（不带参数的 TUI 选择器）的业务域列表来自官方 meta 目录，暂不含 application（v7 API 未收录）；user 授权请显式使用 `--domain application` 或 `--scope application:app_slash_command:read/write`（两者均有效）。待 v7 进入官方目录后 TUI 列表自动出现。
