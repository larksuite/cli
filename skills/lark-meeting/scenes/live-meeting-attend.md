...
编排应用机器人的完整会中流程，以及用户身份的会议生命周期操作。应用机器人使用 `--as bot`；用户身份使用 `--as user`，当前支持发起、加入、邀请指定用户和离会。
...
| 用户明确要求以用户身份入会 | 使用 `+meeting-join --as user` |
| 用户明确要求以用户身份发起日程会议 | 使用 `+meeting-join --as user --action start` |
...
只有用户明确要求应用机器人或用户身份发起、加入、旁听或代参会时才执行。输入是 9 位会议号，不是长整数 `meeting_id`。
...
- 保存返回的 `meeting.id` 和来源身份；后续操作必须沿用该身份，不要在用户身份与应用机器人之间静默切换。
...
- 当前身份必须已在目标会议中。
- `SELECTED` 接收用户 `open_id`；`--as user` 路径只支持 `SELECTED`。
- `ALL_SUGGESTED` 仅支持 `--as bot`，由服务端筛选合格日程参会人。
