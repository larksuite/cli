...
- 当前能力处于灰度/内测。出现 `missing required scope(s)`、`missing_scopes`、`20017` 或 `ErrNotInGray` 时，先确认当前租户和应用已获得内测资格，不直接套用普通用户权限申请流程。
...
- scope、安装和数据范围都正确后仍返回 `20017` / `ErrNotInGray`，再按内测灰度资格处理。
