...
通过 `meeting_id` 让应用机器人或用户身份离开当前身份所在的视频会议。这是一次**写操作**，会实际把当前身份从会议中移出。
...
- 应用机器人使用 `--as bot`。
- 用户身份使用 `--as user`。
- 离会必须沿用发起或入会时的身份，不要在用户身份与应用机器人之间切换。该命令只能让当前身份自己离会，无法强制移出其他参会人。
...
当前应用机器人或用户身份会立刻从参会列表消失；若会议启用了录制/纪要，该身份的参会时段到此截止。只有在用户明确要求退出 / 离开 / 结束参会时才调用；如需要重新入会，再跑 `+meeting-join` 即可（非真正"不可逆"）。
...
| 用户身份不匹配 | 当前 `--as user` 身份不可用 | 检查当前身份，保留错误码和 `log_id`，不要改用 `--as bot` 重试 |
...
## 相关场景
- [应用机器人或用户身份参会与会中互动](../scenes/live-meeting-attend.md)
