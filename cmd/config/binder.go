...
type openclawBinder struct {
	...
	cfg     *openclaw.OpenClawRoot
	rawApps []openclaw.CandidateApp
}
...
func (b *openclawBinder) ListCandidates() ([]Candidate, error) {
	cfg, err := openclaw.ReadOpenClawConfig(b.path)
	if err != nil {
	...
	raw := openclaw.ListCandidateApps(cfg.Channels.Feishu)
	b.cfg = cfg
	b.rawApps = raw
...
func (b *openclawBinder) Build(appID string) (*core.AppConfig, error) {
	...
	secret, err := openclaw.ResolveSecretInput(selected.AppSecret, b.cfg.Secrets, os.Getenv)
	if err != nil {
		return nil, output.ErrWithHint(output.ExitValidation, "openclaw",
