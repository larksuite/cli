...
func TestInteractiveDomainMetadataUsesActiveBrand(t *testing.T) {
	...
	feishuDomains := newDomainResolver(authTestCatalog(t), registered).metadata("en", core.BrandFeishu)
	if !containsDomainMetadata(feishuDomains, "apps") {
		t.Fatal("apps domain is missing for Feishu interactive login")
	}

	larkDomains := newDomainResolver(authTestCatalog(t), registered).metadata("en", core.BrandLark)
