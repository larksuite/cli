...
func doManualUpdate(opts *UpdateOptions, io *cmdutil.IOStreams, cur, latest string, detect selfupdate.DetectResult, updater *selfupdate.Updater) error {
	...
	if detect.Method == selfupdate.InstallPnpm {
		fmt.Fprintf(io.ErrOut, "\nOr install via pnpm (note: skills will not be synced):\n  pnpm add -g %s@%s\n  pnpm dlx skills add larksuite/cli -y -g --copy   # sync skills separately\n", selfupdate.NpmPackage, latest)
	} else {
		fmt.Fprintf(io.ErrOut, "\nOr install via npm (note: skills will not be synced):\n  npm install -g %s@%s\n  npx skills add larksuite/cli -y -g --copy   # sync skills separately\n", selfupdate.NpmPackage, latest)
	}
...
func verificationFailureHint(updater *selfupdate.Updater, latest, pm string) string {
	...
	if pm == "pnpm" {
		return fmt.Sprintf("automatic rollback is unavailable on this platform; reinstall manually (skills will not be synced): pnpm add -g %s@%s && pnpm dlx skills add larksuite/cli -y -g --copy, or download %s", selfupdate.NpmPackage, latest, releaseURL(latest))
	}
	return fmt.Sprintf("automatic rollback is unavailable on this platform; reinstall manually (skills will not be synced): npm install -g %s@%s && npx skills add larksuite/cli -y -g --copy, or download %s", selfupdate.NpmPackage, latest, releaseURL(latest))
}
