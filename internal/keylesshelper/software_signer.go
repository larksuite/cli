...
func (s softwareSigner) unlock(ctx context.Context, directory string, allowCreate bool) (secret []byte, err error) {
	...
	if encoded != "" {
		...
		if err != nil {
			clear(secret)
			return nil, fmt.Errorf("%w: decode software unlock secret: %w", keysigner.ErrCorrupt, err)
		}
		if len(secret) != 32 {
			clear(secret)
			return nil, fmt.Errorf("%w: invalid software unlock secret", keysigner.ErrCorrupt)
		}
		return secret, nil
	}
	if !allowCreate {
