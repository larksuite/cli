...
func TestReadOpenClawConfig_ValidSingleAccount(t *testing.T) {
	...
	data := `{"channels":{"feishu":{"appId":"cli_abc","appSecret":"plain_secret","brand":"feishu"}}}`
	...
	data := `{
		"channels": {
			"feishu": {
				"brand": "feishu",
				"accounts": {
					"work": {"appId": "cli_work", "appSecret": "secret_work", "brand": "feishu"},
					"personal": {"appId": "cli_personal", "appSecret": "secret_personal", "brand": "lark"}
				}
			}
		}
	}`
...
func TestReadOpenClawConfig_EnvTemplate(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "openclaw.json")
	data := `{"channels":{"feishu":{"appId":"cli_env","appSecret":"${FEISHU_APP_SECRET}","brand":"feishu"}}}`
	if err := os.WriteFile(p, []byte(data), 0o644); err != nil {
		t.Fatalf("write temp file: %v", err)
	}
...
func TestReadOpenClawConfig_SecretRefObject(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "openclaw.json")
	data := `{"channels":{"feishu":{"appId":"cli_ref","appSecret":{"source":"file","provider":"fp","id":"/path"},"brand":"feishu"}}}`
	if err := os.WriteFile(p, []byte(data), 0o644); err != nil {
		t.Fatalf("write temp file: %v", err)
	}
