...
func prepareExecRun(ref *SecretRef, pc *ProviderConfig, getenv func(string) string) (*execRun, error) {
	if pc.Command == "" {
	...
	reqJSON, err := marshalExecRequest(ref)
	if err != nil {
		return nil, err
	}
...
// The ref.Provider name is defaulted upstream in resolveSecretRef, but this
// function still normalizes the unset case to DefaultProviderAlias so it can
// be reasoned about in isolation.
func marshalExecRequest(ref *SecretRef) ([]byte, error) {
...
// package defaults for any non-positive value.
