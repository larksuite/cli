...
func TestSoftwareSignerLifecycle(t *testing.T) {
	...
	for _, ref := range []KeyRef{
		{Label: "es256"},
		{Label: "es384", Algorithm: AlgES384},
		{Label: "es512", Algorithm: AlgES512},
		{Label: "eddsa", Algorithm: AlgEdDSA},
		...
		t.Run(ref.Label, func(t *testing.T) {
			...
			wrongAlgorithm := AlgES256
			if ref.Algorithm == "" || ref.Algorithm == AlgES256 {
				wrongAlgorithm = AlgRS256
			}
