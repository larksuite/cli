...
func TestCredentialProvider_EnvAppSecretMintsTATWhenNoTokenEnvIsSet(t *testing.T) {
	rt := &stubRoundTripper{
		respCode: 200,
		respBody: `{"code":0,"access_token":"tat-from-secret","token_type":"Bearer","expires_in":7200}`,
	}
	cp := NewCredentialProvider(
		[]extcred.Provider{&mockExtProvider{
			name: "env",
	...
	if result.Token != "tat-from-secret" {
		t.Fatalf("ResolveToken() token = %q, want tat-from-secret", result.Token)
	}
	if result.Source != core.CredentialSourceEnv {
		t.Fatalf("ResolveToken() source = %q, want env", result.Source)
	}
	for _, want := range []string{"grant_type=client_credentials", "client_id=ext_app", "client_secret=ext_secret"} {
		if !strings.Contains(rt.gotBody, want) {
			t.Fatalf("request body missing %q: %s", want, rt.gotBody)
		}
	}
}

func TestCredentialProvider_EnvAppSecretTATFallbackRequiresMatchingAppID(t *testing.T) {
	cp := NewCredentialProvider(
		[]extcred.Provider{&mockExtProvider{
			name: "env",
			account: &extcred.Account{
		...
		nil,
		func() (*http.Client, error) {
			t.Fatal("httpClient() should not be called for a different app ID")
			return nil, nil
		},
	)
	...
	var unavailableErr *TokenUnavailableError
	if !errors.As(err, &unavailableErr) {
		t.Fatalf("ResolveToken() error = %T %v, want TokenUnavailableError", err, err)
	}
}
