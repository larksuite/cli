...
var runOpenClawInspect = executeOpenClawInspect
...
func Resolve(ctx context.Context, provider string) (*keylesshelper.Command, error) {
	command, resolved, err := resolve(ctx, provider)
	if err != nil {
		return nil, err
	}
	if resolved != nil {
...
func resolve(ctx context.Context, provider string) (*keylesshelper.Command, *resolvedProvider, error) {
	if ctx == nil {
	...
	resolved, inspectErr := resolveFromInspect(ctx, stateDir, runtime.GOOS, runtime.GOARCH)
	if inspectErr != nil {
