...
func TestTransportSignsBoundRequestsRegardlessOfDestination(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"resource", endpoint + "/open-apis/test?cursor=1", "Bearer fixture-token", true, true, "DPoP fixture-token"},
		{"already_dpop", endpoint + "/open-apis/test", "DPoP fixture-token", true, true, "DPoP fixture-token"},
		{"external", "https://example.com/upload", "Bearer fixture-token", true, true, "DPoP fixture-token"},
		{"unbound", endpoint + "/open-apis/test", "Bearer fixture-token", false, false, "Bearer fixture-token"},
