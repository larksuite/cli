...
1. 修复问题后推送新 commit，CI 会自动重新运行

1. lint 失败通常是代码格式或新引入的代码问题，参照 4.3 节修复
...
Great first contribution! Just a couple of small things to fix.For the test, you can reference `shortcuts/base/record_test.go`for examples of how we structure table-driven tests in this project.
```plaintext
...
1. 建议开启 GitHub Security Advisories
...
1. 检查 tag 是否已存在（本地和远程）

1. 确保 `package.json` 已提交
...
**在PR合入CLI仓库前，需要先合入registry仓库并提交发布！**
...
<grid cols="2">
  ...
  <column width="55">
    ...
    将仓库中最新的skills/lark-目录打包交付QA提测。

  </column>

  <column width="44">

    <image token="" width="492" height="808" align="center"/>
...
<image token="" width="2196" height="938" align="center"/>
...
1. 如果存量问题过多，先修复再启用（参考 `errcheck` 的处理方式）
...
1. `GITHUB_TOKEN` 权限是否充足

1. `fetch_meta.py` 是否在 before hook 中正常执行

1. Go 版本是否匹配（release workflow 使用 Go 1.23）
...
1. 如果是间接依赖，使用 `go get <indirect-dep>@<safe-version>` 升级
