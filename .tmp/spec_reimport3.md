...
<lark-table rows="3" cols="3" header-row="true" column-widths="264,180,244">
  ...
  <lark-tr>
    <lark-td>
      https://code.byted.org/lark/larksuite-cli-registry（内部，不开源）
    </lark-td>
    <lark-td>
      生成逻辑 + OAPI 元数据
    </lark-td>
...
**核心原则**：CLI 仓库零侵入，所有生成逻辑和配置都在 registry 仓库。产物通过 `make` 直接写入 CLI 仓库。meta_data服务端下发，**不开源。**
...
注意！！不要将测试的meta_data.json发布到CLI开源仓库！
