...
// ── buildConditionBody: plain string expected fallback ───────────────────────

func TestCreateFilterViewConditionPlainStringExpected(t *testing.T) {
	f, stdout, _, reg := cmdutil.TestFactory(t, sheetsTestConfig())
	stub := &httpmock.Stub{
		Method: "POST", URL: "/open-apis/sheets/v3/spreadsheets/shtTOKEN/sheets/sheet1/filter_views/fv1/conditions",
		Body: map[string]interface{}{"code": 0, "msg": "success", "data": map[string]interface{}{
			"condition": map[string]interface{}{"condition_id": "A"},
		}},
	}
	reg.Register(stub)
	err := mountAndRunSheets(t, SheetCreateFilterViewCondition, []string{
	...
	if !ok || len(expected) != 1 || expected[0] != "hello" {
		t.Fatalf("expected plain string fallback to [\"hello\"], got: %#v", body["expected"])
	}
}
