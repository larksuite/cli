...
var flagDefs = map[string]commandDef{
	...
	"+cells-get": {
		...
		Flags: []flagDef{
			...
			{Name: "conditional-format", Kind: "own", Type: "bool", Required: "optional", Desc: "Merge conditional format (colorScale/aboveAverage etc.) results into cell_styles (default: false). Requires --include style to be effective"},
			{Name: "dry-run", Kind: "system", Type: "bool", Required: "optional"},
		},
	},
	"+cells-merge": {
	...
	"+cond-format-result-get": {
		...
		Flags: []flagDef{
			...
			{Name: "sheet-name", Kind: "public", Type: "string", Required: "xor", Desc: "Sheet name (XOR with `--sheet-id`)"},
			{Name: "range", Kind: "own", Type: "string", Required: "required", Desc: "A1 range, e.g. `A1:F10` (no sheet prefix — use `--sheet-id` / `--sheet-name` to select the sheet)"},
			{Name: "include", Kind: "own", Type: "string_slice", Required: "optional", Desc: "Comma-separated info categories to include. Must include `style` to see conditional format results", Enum: []string{"value", "formula", "style", "comment", "data_validation"}},
