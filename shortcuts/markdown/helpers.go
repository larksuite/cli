...
func markdownSourceBytes(runtime *common.RuntimeContext, spec markdownUploadSpec) ([]byte, error) {
	if spec.ContentSet {
		return []byte(spec.Content), nil
	}
	if strings.TrimSpace(spec.FilePath) == "" {
		return nil, common.FlagErrorf("--file cannot be empty")
	}
	...
	if err != nil {
		return nil, output.ErrValidation("cannot read file: %s", err)
	}
	return data, nil
}
...
func uploadMarkdownFile(runtime *common.RuntimeContext, spec markdownUploadSpec, payload []byte) (markdownUploadResult, error) {
	fileName := finalMarkdownFileName(spec)
	fileSize := int64(len(payload))
	if fileSize > markdownSinglePartSizeLimit {
		return uploadMarkdownFileMultipart(runtime, spec, payload, fileName, fileSize)
	}
	return uploadMarkdownFileAll(runtime, spec, payload, fileName, fileSize)
}

func uploadMarkdownFileAll(runtime *common.RuntimeContext, spec markdownUploadSpec, payload []byte, fileName string, fileSize int64) (markdownUploadResult, error) {
	...
	fd.AddFile("file", bytes.NewReader(payload))
...
func uploadMarkdownFileMultipart(runtime *common.RuntimeContext, spec markdownUploadSpec, payload []byte, fileName string, fileSize int64) (markdownUploadResult, error) {
	prepareBody := map[string]interface{}{
		"file_name":   fileName,
		"parent_type": "explorer",
		"parent_node": spec.FolderToken,
	...
	if err := uploadMarkdownMultipartParts(runtime, payload, session); err != nil {
		return markdownUploadResult{}, err
	}
...
func uploadMarkdownMultipartParts(runtime *common.RuntimeContext, payload []byte, session markdownMultipartSession) error {
	...
	for seq := 0; seq < session.BlockNum; seq++ {
		...
		n, readErr := io.ReadFull(reader, buffer[:int(chunkSize)])
		if readErr != nil {
