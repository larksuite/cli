...
func TestDriveDownloadEntityFallbackRequiresWikiScope(t *testing.T) {
	t.Setenv("LARKSUITE_CLI_CONFIG_DIR", t.TempDir())
	f, stdout, stderr, reg := cmdutil.TestFactory(t, driveTestConfig())
	...
	err := mountAndRunDrive(t, DriveDownload, []string{"+download", "--wiki-token", "wiki_input", "--output", "never.bin", "--as", "bot"}, f, stdout)
