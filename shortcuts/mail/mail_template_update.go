...
var MailTemplateUpdate = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "attach", Type: "string_array", Desc: "Additional non-inline attachment file path. Repeat --attach or pass a comma-separated path list in one occurrence; each file is uploaded to Drive and appended in flag order."},
		{Name: "inline", Type: "string_array", Desc: "Additional inline images as a JSON object or array per occurrence. Repeat --inline to append more images; values are not comma-split. Example value: '{\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}'. file_path must be relative. Reference it from HTML as <img src=\"cid:<unique-id>\">. CID must be unique, e.g. a random hex string."},
	},
