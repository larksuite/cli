...
func buildDraftEditPatchTemplate() map[string]interface{} {
	return map[string]interface{}{
		...
		"notes": []string{
			...
			"`set_body` and `set_reply_body` automatically preserve all attachments (normal and large); to remove a large attachment, use `remove_attachment` with `target.token`",
