...
var MailTemplateCreate = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "to", Type: "string_array", Desc: "Optional. Default To recipient email address. Repeat --to or pass a comma-separated address list in one occurrence; quote each value. Display-name format is supported."},
		{Name: "cc", Type: "string_array", Desc: "Optional. Default Cc recipient email address. Repeat --cc or pass a comma-separated address list in one occurrence; quote each value."},
		{Name: "bcc", Type: "string_array", Desc: "Optional. Default Bcc recipient email address. Repeat --bcc or pass a comma-separated address list in one occurrence; quote each value."},
		{Name: "attach", Type: "string_array", Desc: "Optional. Non-inline attachment file path, relative path only. Repeat --attach or pass a comma-separated path list in one occurrence; order is preserved for LARGE/SMALL classification."},
		{Name: "inline", Type: "string_array", Desc: "Optional. Inline images as a JSON object or array per occurrence. Repeat --inline to append more images; values are not comma-split. Example value: '{\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}'. file_path must be relative. Reference it from HTML as <img src=\"cid:<unique-id>\">. CID must be unique, e.g. a random hex string."},
	},
