...
var MailTemplateCreate = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "to", Type: "string_array", Desc: "Optional. Default To recipient list. May be repeated; each occurrence may contain an RFC recipient list. Quote each value."},
		{Name: "cc", Type: "string_array", Desc: "Optional. Default Cc recipient list. May be repeated; each occurrence may contain an RFC recipient list. Quote each value."},
		{Name: "bcc", Type: "string_array", Desc: "Optional. Default Bcc recipient list. May be repeated; each occurrence may contain an RFC recipient list. Quote each value."},
		{Name: "attach", Type: "string_array", Desc: "Optional. Non-inline attachment paths, relative paths only. May be repeated; each occurrence may contain comma-separated paths. Input order is preserved for LARGE/SMALL classification."},
		{Name: "inline", Type: "string_array", Desc: "Optional. Inline images as a JSON object or array. May be repeated; quote each occurrence. Example value: '{\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}'. file_path must be relative. Reference it from HTML as <img src=\"cid:<unique-id>\">. CID must be unique, e.g. a random hex string."},
	},
