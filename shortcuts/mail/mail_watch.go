...
var MailWatch = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "max-events", Type: "int", Desc: "Exit after N successful emits (0 = unlimited). Multi-worker EventKeys may emit up to workers-1 past N before all workers stop."},
		{Name: "timeout", Type: "duration", Desc: "Exit after DURATION (e.g. 30s, 2m). 0 = no timeout. Timeout is a normal exit (code 0; stderr 'reason: timeout')."},
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if outputDir != "" {
			...
			if err := vfs.MkdirAll(safePath, 0700); err != nil {
