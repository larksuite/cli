...
// automatically preserves the large attachment card if the draft has
// one and the user's new value has no card in its user-authored region
// (i.e. before any quote block the user included). This mirrors how
// normal attachments (MIME parts) survive body edits.
func setBody(snapshot *DraftSnapshot, value string, options PatchOptions) error {
	value = autoPreserveLargeAttachmentCard(snapshot, value)
	switch {
...
// autoPreserveLargeAttachmentCard extracts the existing large attachment
// card from the draft's HTML body and injects it into value (before any
// quote block in value, or appended). No-op when:
//   - the draft has no card to preserve, or
//   - the user's value already supplies a card in its user-authored region
//     (before any quote block in value). Cards found inside a quote block
//     in value belong to the quoted original message, not the user, so
//     they are not treated as "user-supplied".
func autoPreserveLargeAttachmentCard(snapshot *DraftSnapshot, value string) string {
	htmlPart := findPart(snapshot.Body, snapshot.PrimaryHTMLPartID)
	if htmlPart == nil {
		return value
	}
	_, card, _ := SplitAtLargeAttachment(string(htmlPart.Body))
	if card == "" {
		return value
	}
	valuePreQuote, _ := SplitAtQuote(value)
	if HTMLContainsLargeAttachment(valuePreQuote) {
		return value // user supplied their own card in the user region, respect it
	}
	return InsertBeforeQuoteOrAppend(value, card)
}

// setReplyBody replaces only the user-authored portion of the HTML body,
// preserving the trailing reply/forward quote block (generated by
// +reply / +forward). It delegates card preservation to setBody, which
// handles it automatically via autoPreserveLargeAttachmentCard. When
// there is no quote block, it behaves identically to setBody.
