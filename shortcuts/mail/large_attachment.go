...
// insertBeforeQuoteOrAppend inserts block into html before the outermost
// quote wrapper (identified by class="history-quote-wrapper"), or appends
// it to the end if no quote block is found.
//
// We match on the wrapper class rather than id="lark-mail-quote" because
// the wrapper is the outermost quote container generated by the CLI, while
// lark-mail-quote ids can appear deeply nested inside the quoted content
// from previous replies in the thread.
func insertBeforeQuoteOrAppend(html, block string) string {
	idx := strings.Index(html, draftpkg.QuoteWrapperClass)
	if idx < 0 {
		return html + block
	}
