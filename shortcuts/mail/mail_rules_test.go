...
func TestMailRuleReorderShortcutPostsFullAndMoveOrders(t *testing.T) {
	...
	t.Run("partial order completes missing ids", func(t *testing.T) {
		f, stdout, _, reg := mailShortcutTestFactory(t)
		reg.Register(mailRuleListStub(
			mailRuleTestRawRule("a", "A"),
			mailRuleTestRawRule("b", "B"),
...
func TestMailRuleOrderValidationErrors(t *testing.T) {
	if completed, err := completeRuleOrder([]string{"a"}, []string{"a", "b"}); err != nil || strings.Join(completed, ",") != "a,b" {
		t.Fatalf("completeRuleOrder() = %v, %v; want a,b with nil error", completed, err)
	}
	if err := validatePartialRuleOrder([]string{"a", "a"}, []string{"a", "b"}); err == nil {
