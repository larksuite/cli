...
var MailRuleReorder = common.Shortcut{
	...
	HasFormat:   true,
	Flags: append([]common.Flag{}, append(mailRuleCommonFlags,
		common.Flag{Name: "rule-ids", Type: "string_slice", Desc: "Target rule ID order. Missing current rules are appended in their existing order."},
...
func buildRuleTargetOrder(rt *common.RuntimeContext, current []mailRuleEnvelope) ([]string, error) {
	...
	if ids := normalizeRuleIDs(rt.StrSlice("rule-ids")); len(ids) > 0 {
		target, err := buildCompletedRuleOrder(ids, currentIDs)
		if err != nil {
			return nil, err
		}
		return target, nil
...
func buildCompletedRuleOrder(target, current []string) ([]string, error) {
