...
var MailRuleReorder = common.Shortcut{
	...
	HasFormat:   true,
	Flags: append([]common.Flag{}, append(mailRuleCommonFlags,
		common.Flag{Name: "rule-ids", Type: "string_slice", Desc: "Target rule ID order. Missing current rules are appended in their existing relative order."},
...
func completeRuleTargetOrder(target, current []string) ([]string, error) {
	...
	for _, id := range target {
		...
		if _, ok := want[id]; !ok {
			return nil, mailValidationParamError("--rule-ids", "--rule-ids contains unknown rule id %s; run +rule-list first and submit current rule ids", id)
		}
		seen[id] = struct{}{}
		out = append(out, id)
	}
