...
var MailRuleReorder = common.Shortcut{
	...
	HasFormat:   true,
	Flags: append([]common.Flag{}, append(mailRuleCommonFlags,
		common.Flag{Name: "rule-ids", Type: "string_slice", Desc: "Rule IDs to place first in the given order. Unspecified current rules keep their relative order afterward."},
...
func completeRuleOrder(explicit, current []string) ([]string, error) {
	if len(current) == 0 {
		return nil, mailValidationParamError("--rule-ids", "current mailbox has no rules to reorder")
	}
	...
	for _, rawID := range explicit {
		id := strings.TrimSpace(rawID)
		if id == "" {
			return nil, mailValidationParamError("--rule-ids", "--rule-ids contains an empty rule id")
		}
		if strings.ContainsAny(id, " \t\r\n") {
			return nil, mailValidationParamError("--rule-ids", "--rule-ids contains an invalid rule id %q", id)
		}
		if _, ok := seen[id]; ok {
			return nil, mailValidationParamError("--rule-ids", "--rule-ids contains duplicate rule id %q", id)
		}
		if _, ok := available[id]; !ok {
			return nil, mailValidationParamError("--rule-ids", "rule id %q does not belong to the current mailbox; run +rule-list first", id)
		}
		seen[id] = struct{}{}
