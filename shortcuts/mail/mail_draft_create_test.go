...
func TestBuildRawEMLForDraftCreate_AutoResolveCountedInSizeLimit(t *testing.T) {
	...
	_, _, _, err := buildRawEMLForDraftCreate(context.Background(), newRuntimeWithFrom("sender@example.com"), input, nil, "", nil, "", "", nil, nil, composeSenderInfo{})
	if err == nil {
...
func TestBuildRawEMLForDraftCreate_WithPriority(t *testing.T) {
	...
	rawEML, _, _, err := buildRawEMLForDraftCreate(context.Background(), newRuntimeWithFrom("sender@example.com"), input, nil, "1", nil, "", "", nil, nil, composeSenderInfo{})
	if err != nil {
		t.Fatalf("buildRawEMLForDraftCreate() error = %v", err)
	}
	eml := decodeBase64URL(rawEML)
...
func TestBuildRawEMLForDraftCreate_RequestReceiptAddsHeader(t *testing.T) {
	...
	rawEML, _, _, err := buildRawEMLForDraftCreate(context.Background(),
		newRuntimeWithFromAndRequestReceipt("sender@example.com", true), input, nil, "", nil, "", "", nil, nil, composeSenderInfo{})
	if err != nil {
		t.Fatalf("buildRawEMLForDraftCreate() error = %v", err)
	}
	eml := decodeBase64URL(rawEML)
...
func TestBuildRawEMLForDraftCreate_RequestReceiptOmittedByDefault(t *testing.T) {
	...
	rawEML, _, _, err := buildRawEMLForDraftCreate(context.Background(),
		newRuntimeWithFromAndRequestReceipt("sender@example.com", false), input, nil, "", nil, "", "", nil, nil, composeSenderInfo{})
	if err != nil {
		t.Fatalf("buildRawEMLForDraftCreate() error = %v", err)
	}
	eml := decodeBase64URL(rawEML)
...
func TestBuildRawEMLForDraftCreate_WithCalendarEvent(t *testing.T) {
	...
	rawEML, _, _, err := buildRawEMLForDraftCreate(context.Background(), rt, input, nil, "", nil, "", "", nil, nil, composeSenderInfo{})
	if err != nil {
		t.Fatalf("buildRawEMLForDraftCreate() error = %v", err)
	}
	eml := decodeBase64URL(rawEML)
