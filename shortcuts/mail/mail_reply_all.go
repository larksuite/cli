...
var MailReplyAll = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "to", Type: "string_array", Desc: "Additional To recipients. May be repeated; each occurrence may contain an RFC recipient list. Quote each value; appended to original recipients."},
		{Name: "cc", Type: "string_array", Desc: "Additional CC recipients. May be repeated; each occurrence may contain an RFC recipient list. Quote each value."},
		{Name: "bcc", Type: "string_array", Desc: "BCC recipients. May be repeated; each occurrence may contain an RFC recipient list. Quote each value."},
		{Name: "remove", Type: "string_array", Desc: "Addresses to exclude from the outgoing reply. May be repeated; each occurrence may contain comma-separated addresses. Quote each value; input order is preserved."},
		{Name: "plain-text", Type: "bool", Desc: "Force plain-text mode, ignoring all HTML auto-detection. Cannot be used with --inline."},
		{Name: "attach", Type: "string_array", Desc: "Attachment file paths, relative paths only. May be repeated; each occurrence may contain comma-separated paths. Quote each value; input order is preserved."},
