...
var MailDraftCreate = common.Shortcut{
	...
	HasFormat:   true,
	Flags: []common.Flag{
		{Name: "to", Type: "string_array", Desc: "Optional. To recipient email address. Repeat --to or pass a comma-separated address list in one occurrence; quote each value. Display-name format is supported. When omitted, the draft is created without recipients (they can be added later via +draft-edit)."},
		...
		{Name: "cc", Type: "string_array", Desc: "Optional. Cc recipient email address. Repeat --cc or pass a comma-separated address list in one occurrence; quote each value. Display-name format is supported."},
		{Name: "bcc", Type: "string_array", Desc: "Optional. Bcc recipient email address. Repeat --bcc or pass a comma-separated address list in one occurrence; quote each value. Display-name format is supported."},
		{Name: "plain-text", Type: "bool", Desc: "Force plain-text mode, ignoring HTML auto-detection. Cannot be used with --inline."},
		{Name: "attach", Type: "string_array", Desc: "Optional. Regular attachment file path, relative path only. Repeat --attach or pass a comma-separated path list in one occurrence; quote each value. Order is preserved and each path must point to a readable local file."},
		{Name: "inline", Type: "string_array", Desc: "Optional. Inline images as a JSON object or array per occurrence. Repeat --inline to append more images; values are not comma-split. Example value: '{\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}'. file_path must be relative. Reference it from HTML as <img src=\"cid:<unique-id>\">. CID must be unique, e.g. a random hex string. Cannot be used with --plain-text."},
