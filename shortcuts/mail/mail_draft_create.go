...
var MailDraftCreate = common.Shortcut{
	...
	HasFormat:   true,
	Flags: []common.Flag{
		{Name: "to", Type: "string_array", Desc: "Optional. To recipient list. May be repeated; each occurrence may contain comma-separated recipients. Quote each value. RFC display-name format, including quoted commas, is supported. When omitted, the draft is created without recipients (they can be added later via +draft-edit)."},
		...
		{Name: "cc", Type: "string_array", Desc: "Optional. Cc recipient list. May be repeated; each occurrence may contain comma-separated recipients. Quote each value. RFC display-name format, including quoted commas, is supported."},
		{Name: "bcc", Type: "string_array", Desc: "Optional. Bcc recipient list. May be repeated; each occurrence may contain comma-separated recipients. Quote each value. RFC display-name format, including quoted commas, is supported."},
		{Name: "plain-text", Type: "bool", Desc: "Force plain-text mode, ignoring HTML auto-detection. Cannot be used with --inline."},
		{Name: "attach", Type: "string_array", Desc: "Optional. Regular attachment file paths, relative paths only. May be repeated; each occurrence may contain comma-separated paths. Quote each value; input order is preserved. Each path must point to a readable local file."},
		{Name: "inline", Type: "string_array", Desc: "Optional. Inline images as a JSON object or array. May be repeated; quote each occurrence. Example value: '{\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}'. file_path must be relative. Reference it from HTML as <img src=\"cid:<unique-id>\">. CID must be unique, e.g. a random hex string. Cannot be used with --plain-text."},
