...
func TestAutomationSkillContract_ChangedHandlerStartWaitsForThisRelease(t *testing.T) {
	...
	for _, boundary := range []string{
		...
		"确认 `failed` 时报告发布未通过，原本 enabled 的 trigger 仅在确认新代码未上线后恢复 enabled，原本 disabled 的保持 disabled。",
...
func TestReleaseSkillContract_HandlesKnownAndUnknownStatuses(t *testing.T) {
...
func TestReleaseSkillContract_CreateReasonIsSafeAndConfirmedOnce(t *testing.T) {
	...
	requireInOrder(t, rules,
		...
		"控制字符",
		"U+200B–U+200D 与 U+FEFF 零宽字符",
		"U+202A–U+202E 双向嵌入/覆盖字符",
		"U+2066–U+2069 双向隔离字符",
