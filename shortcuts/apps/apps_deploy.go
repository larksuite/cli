...
var AppsDeploy = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "file-path", Desc: "publish a single HTML file plus the local files it references, transitively (path relative to the current directory); references resolve inside the file's own directory, which becomes the site root, and one climbing above it stops the publish; mutually exclusive with --dir"},
		{Name: "dir", Desc: "publish a whole directory as-is (path relative to the current directory): every file in it ships and no references are followed, so a page pointing outside the directory is reported but still published broken; mutually exclusive with --file-path"},
