...
var BaseDashboardBlockCreate = common.Shortcut{
	...
	Tips: []string{
		...
		"--position is optional precise layout in a 12-col grid; omit it to let the server auto-layout. Coordinate values are not validated locally; how the server treats out-of-range or overlapping coordinates is not verified. To re-tidy an existing dashboard use +dashboard-arrange instead.",
