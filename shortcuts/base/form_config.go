...
var BaseFormSubmitActionsUpdate = common.Shortcut{
	...
	Flags: appendFormConfigFlags(
		...
		common.Flag{Name: "description", Desc: "result page description; Markdown links are supported", Input: []string{common.File, common.Stdin}},
	...
	Tips: []string{
		...
		"Enabling result-page requires --title; omit --description to preserve the current description, or pass an empty string to clear it.",
		"Use Markdown links such as [View document](https://example.com); person mentions are not supported.",
	},
	Validate: func(_ context.Context, runtime *common.RuntimeContext) error {
...
var BaseFormLotteryAction = common.Shortcut{
	...
	Tips: []string{
		...
		"Every award requires a unique position from 0, 1, 2, 3, 5, 6, or 7; array order does not control the lottery layout.",
		"For update, copy award_id from the GET response to update an existing award; omit award_id for a new award. Do not provide award_id when enabling lottery.",
...
func buildFormSubmitActionsBody(runtime *common.RuntimeContext) (map[string]interface{}, error) {
	...
	case "result-page":
		...
		if enabled {
			...
			if runtime.Changed("description") {
				group["description_markdown"] = runtime.Str("description")
			}
		} else if runtime.Changed("title") || runtime.Changed("description") {
			return nil, baseFlagErrorf("disabling result page must not include --title or --description")
		}
		body["result_page"] = group
		return body, nil
	case "redirect":
		if runtime.Changed("title") {
			return nil, baseFlagErrorf("--title cannot be used with --type redirect")
		}
		if runtime.Changed("description") {
			return nil, baseFlagErrorf("--description cannot be used with --type redirect")
		}
		if enabled {
			if runtime.Str("redirect-url") == "" {
				return nil, baseFlagErrorf("--redirect-url is required when redirect is enabled")
			}
			if !isHTTPOrHTTPSFormConfigURL(runtime.Str("redirect-url")) {
				return nil, baseFlagErrorf("--redirect-url must be a valid HTTP or HTTPS URL")
			}
...
func buildFormLotteryActionBody(runtime *common.RuntimeContext) (map[string]interface{}, error) {
	...
	if config != "" {
		...
		if action == "enable" || action == "update" {
			if err := validateLotteryConfig(lottery, action); err != nil {
				return nil, err
			}
		}
...
func validateLotteryConfig(lottery *formLotteryPayload, action string) error {
	if action == "update" {
	...
	awardIDs := make(map[string]bool, len(awards))
	awardPositions := make(map[int64]bool, len(awards))
	for index, award := range awards {
		if award.Position == nil {
			return baseFlagErrorf("--config-json.awards[%d].position is required", index)
		}
		position := *award.Position
		switch position {
		case 0, 1, 2, 3, 5, 6, 7:
		default:
			return baseFlagErrorf("--config-json.awards[%d].position must be one of 0, 1, 2, 3, 5, 6, or 7", index)
		}
		if awardPositions[position] {
			return baseFlagErrorf("--config-json award position values must be unique")
		}
		awardPositions[position] = true
		if award.AwardID != nil {
			if action == "enable" {
				return baseFlagErrorf("--config-json.awards[%d].award_id must not be provided for enable action", index)
			}
			awardID := strings.TrimSpace(*award.AwardID)
			if awardID == "" {
				return baseFlagErrorf("--config-json.awards[%d].award_id must be a non-empty string when provided", index)
			}
			if awardIDs[awardID] {
				return baseFlagErrorf("--config-json award_id values must be unique")
			}
			awardIDs[awardID] = true
...
func isHTTPOrHTTPSFormConfigURL(value string) bool {
