...
var BaseFieldUpdate = common.Shortcut{
	...
	Tips: []string{
		...
		`Example button rebind: create the workflow first, then run lark-cli base +field-update --base-token <base_token> --table-id <table_id> --field-id "Sync to CRM" --json '{"name":"Sync to CRM","type":"button","button":{"title":"Sync to CRM","color":0},"trigger":{"type":"automation","workflow_id":"wkf_xxx"}}' --yes`,
