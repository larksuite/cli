...
func TestFormSubmissionSettingsUpdateBuildsBodies(t *testing.T) {
	startAt := futureFormConfigRFC3339(24 * time.Hour)
	endAt := futureFormConfigRFC3339(48 * time.Hour)
	tests := []struct {
		name  string
		flags []string
	...
	}{
		{
			...
			want: map[string]interface{}{"submission_period": map[string]interface{}{
				"enabled": true, "start_at": startAt, "end_at": endAt, "timezone": "Asia/Shanghai",
			}},
		},
		{
...
func TestFormNotificationsUpdateBuildsScheduledBody(t *testing.T) {
	notifyTime := futureFormConfigRFC3339(24 * time.Hour)
...
func TestFormSubmitActionsUpdateBuildsResultPageBody(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "Markdown text and document link",
			flags: []string{"--description", "thanks, [view document](https://example.com/document)"},
			want: map[string]interface{}{
				"revision": float64(123),
				"result_page": map[string]interface{}{
					"enabled":              true,
					"title":                "submitted",
					"description_markdown": "thanks, [view document](https://example.com/document)",
				},
			},
		},
		{
		...
		{
			name: "omitted description preserves it",
			want: map[string]interface{}{
				"revision": float64(123),
				"result_page": map[string]interface{}{
					"enabled": true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags = append(flags, tt.flags...)
			stub := runFormConfigPatch(t, BaseFormSubmitActionsUpdate, "+form-submit-actions-update", "submit-actions", flags...)
			if got := decodeCapturedJSONBody(t, stub); !reflect.DeepEqual(got, tt.want) {
				t.Fatalf("request body=%#v, want %#v", got, tt.want)
			}
		})
...
func TestFormConfigPatchDryRunMatchesExecute(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			flags: []string{"--type", "redirect", "--enabled=true", "--revision", "123", "--redirect-url", "http://example.com/done"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestFormConfigRejectsUnsupportedWriteFields(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			shortcut: BaseFormNotificationsUpdate,
			command:  "+form-notifications-update",
			args:     []string{"--type", "scheduled", "--enabled=true", "--receiver-open-id", "ou_1", "--notify-time", futureFormConfigRFC3339(24 * time.Hour), "--repeat-type", "day", "--timezone", "Not/AZone"},
			param:    "--timezone",
		...
		},
		{
			...
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{"icon_token":"img_1","awards":[]}`},
			param:    "--config-json",
			want:     "icon_token",
		},
		{
			...
			shortcut: BaseFormLotteryAction,
			command:  "+form-lottery-action",
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{}`},
			param:    "--config-json",
			want:     "probability",
		},
		{
			...
			shortcut: BaseFormLotteryAction,
			command:  "+form-lottery-action",
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{"probability":100,"awarder_info":{"name":"owner","contact_info":"contact"},"awards":[{"name":"gift","quantity":1}],"unsupported":true}`},
			param:    "--config-json",
			want:     "unsupported",
		},
		{
			...
			shortcut: BaseFormLotteryAction,
			command:  "+form-lottery-action",
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{"probability":100,"awarder_info":{"name":"owner","contact_info":"contact","unsupported":true},"awards":[{"name":"gift","quantity":1}]}`},
			param:    "--config-json",
			want:     "unsupported",
		},
		{
			...
			shortcut: BaseFormLotteryAction,
			command:  "+form-lottery-action",
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{"probability":100,"awarder_info":{"name":"owner","contact_info":"contact"},"awards":[{"name":"gift","quantity":1}],"winning_table":{"table_id":"tbl_win","status":"linked"}}`},
			param:    "--config-json",
			want:     "winning_table",
		},
		{
		...
		{
			name:     "redirect rejects result page description",
			shortcut: BaseFormSubmitActionsUpdate,
			command:  "+form-submit-actions-update",
			args:     []string{"--type", "redirect", "--enabled=true", "--revision", "123", "--description", "[view document](https://example.com/document)", "--redirect-url", "https://example.com/done"},
			param:    "--description",
			want:     "cannot be used with --type redirect",
		},
		{
			name:     "redirect requires http or https",
			shortcut: BaseFormSubmitActionsUpdate,
			command:  "+form-submit-actions-update",
			args:     []string{"--type", "redirect", "--enabled=true", "--revision", "123", "--redirect-url", "ftp://example.com"},
			param:    "--redirect-url",
			want:     "HTTP or HTTPS",
		},
		{
			...
			shortcut: BaseFormLotteryAction,
			command:  "+form-lottery-action",
			args:     []string{"--action", "enable", "--agree-lottery-terms=true", "--config-json", `{"probability":100,"awarder_info":{"name":"owner","contact_info":"contact"},"awards":[{"name":"gift","quantity":1},{"name":"gift","quantity":2}]}`},
			param:    "--config-json",
			want:     "unique",
		},
	}
...
func futureFormConfigRFC3339(offset time.Duration) string {
