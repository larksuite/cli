...
func TestTemplateCenterExecuteShortcuts(t *testing.T) {
	...
	t.Run("malformed categories response", func(t *testing.T) {
		factory, stdout, reg := newExecuteFactory(t)
		reg.Register(&httpmock.Stub{
			Method: "GET",
			URL:    "/open-apis/base/v3/bases/templates/category",
		...
		err := runShortcut(t, BaseTemplateCategories, []string{"+template-categories"}, factory, stdout)
		assertInvalidResponseProblem(t, err)
	})

	t.Run("malformed list response", func(t *testing.T) {
		factory, stdout, reg := newExecuteFactory(t)
		reg.Register(&httpmock.Stub{
			Method: "GET",
			URL:    "/open-apis/base/v3/bases/templates?limit=10",
			Body: map[string]interface{}{
				"code": 0,
				"data": map[string]interface{}{
					"templates": []interface{}{map[string]interface{}{"token": "tpl_token"}},
					"has_more":  "false",
					"offset":    "",
				},
			},
		...
		err := runShortcut(t, BaseTemplateList, []string{"+template-list"}, factory, stdout)
		assertInvalidResponseProblem(t, err)
	})

	t.Run("malformed search response", func(t *testing.T) {
		factory, stdout, reg := newExecuteFactory(t)
		reg.Register(&httpmock.Stub{
			Method: "GET",
			URL:    "/open-apis/base/v3/bases/templates/search?keyword=AI&limit=10",
			Body: map[string]interface{}{
				"code": 0,
				"data": map[string]interface{}{
					"templates": []interface{}{"not_object"},
					"has_more":  false,
					"offset":    "",
				},
			},
		...
		err := runShortcut(t, BaseTemplateSearch, []string{"+template-search", "--keyword", "AI"}, factory, stdout)
		assertInvalidResponseProblem(t, err)
	})
}

func assertInvalidResponseProblem(t *testing.T, err error) {
	t.Helper()
	if err == nil {
	...
	if problem.Category != errs.CategoryInternal || problem.Subtype != errs.SubtypeInvalidResponse {
		t.Fatalf("problem=(%q,%q), want (%q,%q)", problem.Category, problem.Subtype, errs.CategoryInternal, errs.SubtypeInvalidResponse)
	}
}
