...
- **`--as bot`**：以应用身份访问邮箱。需要在飞书开发者后台为应用开通相应权限，否则请求会被拒绝。bot 身份不能使用默认 `--mailbox me`，必须显式传邮箱地址。发送、回复、转发、草稿编辑等发信链路仍仅支持 user 身份；已声明支持 bot 的读取或整理类 shortcut 可使用 bot 身份。

1. 所有邮件写操作（发送、回复、转发、草稿编辑） → 必须使用 `--as user`，未登录时先使用 `lark-cli auth login --domain mail` 进行登录
2. 读取或整理类操作（查看邮件、会话、收件箱列表、移动文件夹、修改标签、软删除会话等） → 推荐使用 `--as user`；如需应用级批量操作（如管理员代操作），可使用支持 bot 的 shortcut 并显式传 `--mailbox <email>`，确保应用已开通对应权限
