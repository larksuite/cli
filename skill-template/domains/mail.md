...
- **发件人黑白名单（Allow / Blocked Sender）**：用户级的发件人信任 / 屏蔽名单（对应飞书客户端「信任 / 屏蔽发件人」），按发件人地址或域名管理，黑白自动互斥。通过 `user_mailbox.allow_senders`（白名单）和 `user_mailbox.blocked_senders`（黑名单）两个资源管理，各支持 `batch_create`（批量加入）、`batch_remove`（按地址批量移除）和 `list`（分页列出，支持 `keyword` 搜索）。用户级名单操作本人邮箱，`user_mailbox_id` 传 `me`；删除按地址而非 id；需要「全部」视图时分别列出白名单与黑名单后聚合。这与管理员 / 租户级的发件人名单是两套独立机制。
