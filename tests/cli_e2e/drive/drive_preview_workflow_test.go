...
// TestDrive_PreviewAndCoverWorkflow exercises the live +preview and +cover
// shortcuts against a real uploaded file. It verifies the list/download flow,
// local artifact writing, and best-effort remote cleanup.
	...
	t.Run("cover list and download", func(t *testing.T) {
		...
		coverResult, err := clie2e.RunCmdWithRetry(ctx, clie2e.Request{
			...
			ShouldRetry: func(result *clie2e.Result) bool {
				...
				return strings.Contains(output, `"http_status":404`) ||
					strings.Contains(output, `"status":404`) ||
					strings.Contains(outputLower, "no artifact")
			},
		})
		require.NoError(t, err)
...
func createDriveFolderOrSkipPermission(t *testing.T, parentT *testing.T, ctx context.Context, name string) string {
	...
	if result.ExitCode != 0 {
		...
		if strings.Contains(stderrLower, "app scope not enabled") ||
			strings.Contains(stderrLower, "space:folder:create") ||
			strings.Contains(stderrLower, "99991672") {
			t.Skipf("skip drive preview/cover workflow due to missing bot scope space:folder:create: %s", result.Stderr)
		}
	}
	result.AssertExitCode(t, 0)
	result.AssertStdoutStatus(t, true)
