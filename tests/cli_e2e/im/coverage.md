...
- Sync-to-chat relation output: two mocked command-boundary regression tests execute `im +messages-mget` through Cobra and assert the final JSON stdout carries `synced_from_thread_reply` / `synced_from_thread` on the chat-level copy and `synced_to_chat_message` on the original thread reply, with the nested upstream `sync_to_chat_info` object and its unknown fields absent from stdout; formatter coverage also verifies the markers survive nested `thread_replies`. A self-contained live E2E is blocked because the CLI exposes no operation that creates a reply with “sync to chat”, so CI cannot deterministically create and clean up the required relation fixture. Manual verification against a production fixture chat covers the live API response until such a fixture is available.
