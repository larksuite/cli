...
- Sync-to-chat relation output: a mocked command-boundary regression test executes `im +messages-mget` through Cobra and asserts the final JSON stdout preserves the typed `sync_to_chat_info`; formatter coverage also verifies relations survive nested `thread_replies`. A self-contained live E2E is blocked because the CLI exposes no operation that creates a reply with “sync to chat”, so CI cannot deterministically create and clean up the required relation fixture. Manual BOE verification covers the live API response until such a fixture is available.
