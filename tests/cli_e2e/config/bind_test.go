...
func writeOpenClawConfig(t *testing.T, openclawHome, appID, appSecret, brand string) {
	...
	content := `{"channels":{"feishu":{"appId":"` + appID + `","appSecret":"` + appSecret + `","brand":"` + brand + `"}}}`
...
func TestBind_Conflict_WithoutForce(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	hermesHome := filepath.Join(t.TempDir(), ".hermes-test")
	t.Setenv("HERMES_HOME", hermesHome)
	writeHermesEnv(t, hermesHome, "cli_e2e_test", "e2e_secret", "feishu")
	...
	require.NoError(t, err)
	assertStderrError(t, result, 2, "bind",
		`workspace "hermes" already bound at `+configPath,
